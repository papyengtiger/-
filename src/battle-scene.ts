  ...
  getBgmLoopPoint(bgmName: string): number {
    switch (bgmName) {
      ...
      case "battle_legendary_birds_galar": //SWSH Galarian Legendary Birds Battle
        return 0.175;
      case "battle_legendary_paradox": //SV Depths of Area Zero Battle
        return 6.442;
      case "battle_legendary_ruinous": //SV Treasures of Ruin Battle
        return 6.333;
      case "battle_legendary_kor_mir": //SV Koraidon & Miraidon Battle
        return 26.05;
      ...
      case "mystery_encounter_fun_and_games": // EoS Guildmaster Wigglytuff
        return 4.78;
      case "mystery_quiz_time": // EoS Guildmaster Wigglytuff
        return 4.78;
      case "Battle_Mystery_Monster": // PMD Boss Battle
        return 0.01;
      case "Battle_against_demonstery": //  EoS Battle against Dusknoir
        return 0.01;
      case "Battle_against_M.N.O_248 & M.N.O_248_2.0": // EoS Palkia's Onslaught!
        return 0.01;
      case "Kecleon_Shop": // EoS Kecleon Shop
        return 0.01;
      case "Stop!_Thief!": // EoS Stop! Thief!
        return 0.01;
      case "monster_house": // EoS MonsterHouse
        return 0.01;
      ...
      for (const partyMember of nonFaintedPartyMembers) {
        ...
        if (participated && pokemonDefeated) {
          ...
          if (
            ...
            // battleContext 및 lastUsedMove를 안전하게 확인 후 기술 위력 증가 적용
            if (typeof battleContext !== "undefined" && typeof lastUsedMove !== "undefined" && lastUsedMove) {
              const boostedPower = powerBoosterModifier.applyMovePowerBoost(
                lastUsedMove,
                partyMember,
                lastOpponent || null, // 상대 포켓몬이 없을 경우 null 전달
                lastUsedMove.getBasePower(), // 원래 기술 위력
                battleContext,
              );
              console.log(
                `Power boosted from ${lastUsedMove.getBasePower()} to ${boostedPower} due to ${powerBoosterModifier.getStackCount()} stack(s).`,
              );
            }
          }

          // 1. 돌격조끼 스택 증가 (구애시리즈 아이템에 영향을 주지 않음)
          const assaultVestModifier = partyMember.getHeldItems().find(m => m instanceof PokemonDefensiveStatModifier);
          if (assaultVestModifier && assaultVestModifier.stackCount < assaultVestModifier.getMaxHeldItemCount()) {
            assaultVestModifier.stackCount++;
            console.log(`Assault Vest stack increased to ${assaultVestModifier.stackCount}`);

            // battleContext 및 상대 포켓몬을 안전하게 확인 후 방어력과 특수방어력 증가 적용
            if (typeof battleContext !== "undefined" && typeof lastOpponent !== "undefined" && lastOpponent) {
              // 방어력과 특수방어력 증가
              const affectedStats = [Stat.DEF, Stat.SPDEF];
              affectedStats.forEach(stat => {
                const statHolder = partyMember.getStatHolder(stat);
                if (statHolder) {
                  assaultVestModifier.apply(partyMember, stat, statHolder);
                  console.log(
                    `${stat} boosted by Assault Vest to ${statHolder.value} due to ${assaultVestModifier.stackCount} stack(s).`,
                  );
                }
              });
            }
          ...
          // 2. 구애시리즈 아이템들의 스택 독립적으로 증가

          // 구애스카프
          const speedStatModifier = partyMember.getHeldItems().find(m => m instanceof SpeedStatModifier);
          if (speedStatModifier && speedStatModifier.stackCount < speedStatModifier.getMaxHeldItemCount()) {
            speedStatModifier.stackCount++;
            console.log(`Speed stat modifier stack increased to ${speedStatModifier.stackCount}`);
          }
          ...
          // 구애머리띠
          const atkStatModifier = partyMember.getHeldItems().find(m => m instanceof AtkStatModifier);
          if (atkStatModifier && atkStatModifier.stackCount < atkStatModifier.getMaxHeldItemCount()) {
            atkStatModifier.stackCount++;
            console.log(`Attack stat modifier stack increased to ${atkStatModifier.stackCount}`);
          }
