...
import { BerryModifier, HitHealModifier, PokemonHeldItemModifier, BoostEnergyModifier, RunSuccessModifier, IgnoreWeatherEffectsItemModifier, MoveAbilityBypassModifier, PreventExplosionItemModifier, SheerForceItemModifier, AbilityGuardItemModifier } from "#modifiers/modifier";
...
export class AbilityGuardPreDefendAbAttr extends PreDefendAbAttr {
...
export class PostSummonReduceOpponentStatsAbAttr extends PostSummonAbAttr {
  private stats: Stat[];
  private multiplier: number;

  constructor(stats: Stat[], multiplier: number) {
    super(true); // PostSummon 시 적용
    this.stats = stats;
    this.multiplier = multiplier;
  }
  ...
  override apply({ pokemon, simulated }: AbAttrBaseParams): void {
    ...
    for (const opponent of opponents) {
      ...
      for (const stat of this.stats) {
        const statVal = new NumberHolder(opponent.getStat(stat));
        const hasApplied = new BooleanHolder(false);

        const fieldMultiply = new FieldMultiplyStatAbAttr(stat, this.multiplier);
        if (fieldMultiply.canApply({ stat, target: opponent, statVal, hasApplied })) {
          fieldMultiply.apply({ stat, target: opponent, statVal, hasApplied });
          // 소수점 버림 처리
          opponent.setStat(stat, Math.floor(statVal.value));
          console.log(` → ${Stat[stat]} after reduction: ${Math.floor(statVal.value)}`);
        }
      }
    }
  }
...
export class PostSummonFormChangeByWeatherAbAttr extends PostSummonAbAttr {
  ...
  override canApply({ pokemon }: AbAttrBaseParams): boolean {
    ...
    if (ignoreWeather) {
      console.log("Utility Umbrella active - Weather-based form change ignored on summon.");
      return false; // 발동 자체가 불가능하도록 처리
    }
  ...
  /**
   * Calls the {@linkcode BattleScene.triggerPokemonFormChange | triggerPokemonFormChange}
   */
  override apply({ pokemon, simulated }: AbAttrBaseParams): void {
    if (!simulated) {
...
export class PostWeatherChangeAddBattlerTagAttr extends PostWeatherChangeAbAttr {
  ...
  override canApply({ weather, pokemon }: PostWeatherChangeAbAttrParams): boolean {
    ...
    if (ignoreWeather) {
      console.log("Utility Umbrella active - Weather-based tag add ignored.");
      return false; // 발동 불가
    }
...
export class PostWeatherLapseHealAbAttr extends PostWeatherLapseAbAttr {
  ...
  override canApply({ pokemon }: PostWeatherLapseAbAttrParams): boolean {
    ...
    if (ignoreWeather) {
      console.log("Utility Umbrella active - Weather-based healing ignored.");
      return false; // 발동 불가
    }
...
export class BoostEnergyTagAttr extends AbAttr {
  ...
  private getTagTypeForAbility(user: Pokemon): BattlerTagType {
    switch (user.abilityId) {
      case AbilityId.PROTOSYNTHESIS:
        return BattlerTagType.PROTOSYNTHESIS;
      case AbilityId.QUARK_DRIVE:
        return BattlerTagType.QUARK_DRIVE;
      case AbilityId.PLUVIAFLUX:
        return BattlerTagType.PLUVIAFLUX;
      case AbilityId.CRYOSYNTHESIS:
        return BattlerTagType.CRYOSYNTHESIS;
      case AbilityId.PHYTONCIDE:
        return BattlerTagType.PHYTONCIDE;
      case AbilityId.PSAMMOSYNTHESIS:
        return BattlerTagType.PSAMMOSYNTHESIS;
      case AbilityId.UNSEEN_FORCE:
        return BattlerTagType.UNSEEN_FORCE;
      case AbilityId.NEURO_CHARGE:
        return BattlerTagType.NEURO_CHARGE;
      default:
...
export class PassiveRegenAbAttr extends PostTurnAbAttr {
  private healFactor: number;
  private targets: Pokemon[] = [];

  constructor(healFactor = 1) {
    super(true); // PostTurnAbAttr의 "모든 아군 고려" 옵션을 true로 설정
    this.healFactor = healFactor;
  }

  override canApply({ pokemon }: PostTurnAbAttrParams): boolean {
    this.targets = [];
    ...
    // 아군
    const ally = pokemon.getAlly();
    if (ally && !ally.isFullHp()) {
      this.targets.push(ally);
    }
  ...
  override apply({ pokemon, passive, simulated }: PostTurnAbAttrParams): void {
    ...
    for (const target of this.targets) {
      const healAmount = toDmgValue(target.getMaxHp() / (16 / this.healFactor));
...
export class BypassFreezeDamageReductionAbAttr extends AbAttr {
  constructor() {
    super(false);
  }
...
export class PostFaintContactDamageAbAttr extends PostFaintAbAttr {
  ...
  override canApply({ pokemon, attacker, move, simulated }: PostFaintAbAttrParams): boolean {
    ...
    // Magic Guard 등 간접피해 무효화 어빌리티 검사
...
const AbilityAttrs = Object.freeze({
  ...
  BypassFreezeDamageReductionAbAttr,
  PassiveRegenAbAttr,
  PostSummonReduceOpponentStatsAbAttr,
  BoostEnergyTagAttr,
...
export function initAbilities() {
  allAbilities.push(
    ...
    new Ability(AbilityId.MAGMA_ARMOR, 3)
      ...
      .attr(StatusEffectImmunityAbAttr, StatusEffect.FROSTBITE)
      .attr(ReceivedTypeDamageMultiplierAbAttr, PokemonType.WATER, 0.5)
    ...
    new Ability(AbilityId.PRESSURE, 3)
      ...
      .attr(PostSummonReduceOpponentStatsAbAttr, [ Stat.ATK, Stat.DEF, Stat.SPATK, Stat.SPDEF, Stat.SPD ], 0.9)
    ...
    new Ability(AbilityId.HYPER_CUTTER, 3)
      ...
      .attr(MovePowerBoostAbAttr, (user, target, move) => move.hasFlag(MoveFlags.MAKES_CONTACT), 1.2)
      .ignorable(),
    ...
    new Ability(AbilityId.PURE_POWER, 3)
      .attr(StatMultiplierAbAttr, Stat.SPATK, 2),
    new Ability(AbilityId.SHELL_ARMOR, 3)
      .attr(BlockCritAbAttr)
      .attr(PostStatStageChangeStatStageChangeAbAttr, (target, statsChanged, stages) => stages < 0, [ Stat.SPDEF ], 1)
      .ignorable(),
    ...
    new Ability(AbilityId.MOTOR_DRIVE, 4)
      .attr(TypeImmunityStatStageChangeAbAttr, PokemonType.ELECTRIC, [ Stat.ATK, Stat.SPATK, Stat.SPD ], 1)
      .ignorable(),
    ...
    new Ability(AbilityId.SLOW_START, 4)
      .attr(PostSummonAddBattlerTagAbAttr, BattlerTagType.SLOW_START, 3),
    ...
    new Ability(AbilityId.TELEPATHY, 5)
      ...
      .attr(MoveTypePowerBoostAbAttr, PokemonType.PSYCHIC, 1.3)
      .ignorable(),
    ...
    new Ability(AbilityId.POISON_TOUCH, 5)
      .attr(MoveTypePowerBoostAbAttr, PokemonType.POISON, 1.3)
      .attr(PostAttackContactApplyStatusEffectAbAttr, 30, StatusEffect.POISON),
    new Ability(AbilityId.REGENERATOR, 5)
      .attr(PreSwitchOutHealAbAttr),
    new Ability(AbilityId.BIG_PECKS, 5)
      .attr(ProtectStatAbAttr, Stat.DEF)
      .attr(ReceivedMoveDamageMultiplierAbAttr, (target, user, move) => move.hasFlag(MoveFlags.MAKES_CONTACT), 0.75)
      .ignorable(),
    ...
    new Ability(AbilityId.MUMMY, 5)
      ...
      .attr(MoveTypePowerBoostAbAttr, PokemonType.GHOST, 1.3)
      .bypassFaint(),
    ...
    new Ability(AbilityId.IRON_BARBS, 5)
      .attr(PostDefendContactDamageAbAttr, 8)
      .attr(MoveTypePowerBoostAbAttr, PokemonType.STEEL, 1.2)
      .bypassFaint(),
    ...
    new Ability(AbilityId.TURBOBLAZE, 5)
      ...
      .attr(MoveAbilityBypassAbAttr)
      .attr(MoveTypePowerBoostAbAttr, PokemonType.FIRE, 1.5)
      .attr(MoveTypePowerBoostAbAttr, PokemonType.DRAGON, 1.5),
    new Ability(AbilityId.TERAVOLT, 5)
      .attr(PostSummonMessageAbAttr, (pokemon: Pokemon) => i18next.t("abilityTriggers:postSummonTeravolt", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }))
      .attr(MoveTypePowerBoostAbAttr, PokemonType.DRAGON, 1.5)
      .attr(MoveTypePowerBoostAbAttr, PokemonType.ELECTRIC, 1.5)
      .attr(MoveAbilityBypassAbAttr),
    ...
    new Ability(AbilityId.REFRIGERATE, 6)
      .attr(MoveTypeChangeAbAttr, PokemonType.ICE, 1.3, (_user, _target, move) => move.type === PokemonType.NORMAL),
    ...
    new Ability(AbilityId.PIXILATE, 6)
      .attr(MoveTypeChangeAbAttr, PokemonType.FAIRY, 1.3, (_user, _target, move) => move.type === PokemonType.NORMAL),
    ...
    new Ability(AbilityId.AERILATE, 6)
      .attr(MoveTypeChangeAbAttr, PokemonType.FLYING, 1.3, (_user, _target, move) => move.type === PokemonType.NORMAL),
    ...
    new Ability(AbilityId.DELTA_STREAM, 6)
      ...
      .attr(MoveTypePowerBoostAbAttr, PokemonType.FLYING, 1.3)
      .attr(MoveTypePowerBoostAbAttr, PokemonType.DRAGON, 1.3)
      .attr(PreLeaveFieldClearWeatherAbAttr)
      .bypassFaint(),
    ...
    new Ability(AbilityId.BERSERK, 7)
      .attr(PostDefendHpGatedStatStageChangeAbAttr, (_target, _user, move) => move.category !== MoveCategory.STATUS, 0.5, [ Stat.SPATK ], 3)
      .condition(getSheerForceHitDisableAbCondition()),
    ...
    new Ability(AbilityId.GALVANIZE, 7)
      .attr(MoveTypeChangeAbAttr, PokemonType.ELECTRIC, 1.3, (_user, _target, move) => move.type === PokemonType.NORMAL),
    ...
    new Ability(AbilityId.RECEIVER, 7)
      ...
      .attr(MoveTypePowerBoostAbAttr, PokemonType.FIGHTING, 1.3)
      .uncopiable(),
    ...
    new Ability(AbilityId.PROPELLER_TAIL, 8)
      .attr(MoveTypePowerBoostAbAttr, PokemonType.WATER, 1.2)
      .attr(ChangeMovePriorityAbAttr, (pokemon, move) => pokemon.getMoveType(move) === PokemonType.WATER, 1),
    ...
    new Ability(AbilityId.STEAM_ENGINE, 8)
      ...
      }, [ Stat.ATK, Stat.SPATK, Stat.SPD ], 3),
    ...
    new Ability(AbilityId.LINGERING_AROMA, 9)
      ...
      .attr(PostSummonStatStageChangeAbAttr, [ Stat.EVA ], -1, false, true)
      .bypassFaint(),
 ...
 new Ability(AbilityId.BRUTALIZE, 224)
      .attr(MoveTypeChangeAbAttr, PokemonType.FIGHTING, 1.3, (_user, _target, move) => move.type === PokemonType.NORMAL),
   new Ability(AbilityId.INFERNALIZE, 224)
      .attr(MoveTypeChangeAbAttr, PokemonType.FIRE, 1.3, (_user, _target, move) => move.type === PokemonType.NORMAL),
   new Ability(AbilityId.NATURALIZE, 224)
      .attr(MoveTypeChangeAbAttr, PokemonType.GRASS, 1.3, (_user, _target, move) => move.type === PokemonType.NORMAL),
   new Ability(AbilityId.HYDRALIZE, 224)
      .attr(MoveTypeChangeAbAttr, PokemonType.WATER, 1.3, (_user, _target, move) => move.type === PokemonType.NORMAL),
   new Ability(AbilityId.STEELIZE, 224)
      .attr(MoveTypeChangeAbAttr, PokemonType.STEEL, 1.3, (_user, _target, move) => move.type === PokemonType.NORMAL),
   new Ability(AbilityId.PSYCHLIZE, 224)
      .attr(MoveTypeChangeAbAttr, PokemonType.PSYCHIC, 1.3, (_user, _target, move) => move.type === PokemonType.NORMAL),
   new Ability(AbilityId.DRACONIZE, 224)
      .attr(MoveTypeChangeAbAttr, PokemonType.DRAGON, 1.3, (_user, _target, move) => move.type === PokemonType.NORMAL),
   new Ability(AbilityId.SPECTRALIZE, 224)
      .attr(MoveTypeChangeAbAttr, PokemonType.GHOST, 1.3, (_user, _target, move) => move.type === PokemonType.NORMAL),
   new Ability(AbilityId.SINISTERIZE, 224)
      .attr(MoveTypeChangeAbAttr, PokemonType.DARK, 1.3, (_user, _target, move) => move.type === PokemonType.NORMAL),
   new Ability(AbilityId.CARAPALATE, 224)
      .attr(MoveTypeChangeAbAttr, PokemonType.BUG, 1.3, (_user, _target, move) => move.type === PokemonType.NORMAL),
   new Ability(AbilityId.TOXILATE, 224)
      .attr(MoveTypeChangeAbAttr, PokemonType.POISON, 1.3, (_user, _target, move) => move.type === PokemonType.NORMAL),
   new Ability(AbilityId.TERRALIZE, 224)
      .attr(MoveTypeChangeAbAttr, PokemonType.GROUND, 1.3, (_user, _target, move) => move.type === PokemonType.NORMAL),
   new Ability(AbilityId.MINERALIZE, 224)
      .attr(MoveTypeChangeAbAttr, PokemonType.ROCK, 1.3, (_user, _target, move) => move.type === PokemonType.NORMAL),
   new Ability(AbilityId.PRIDE, 224)
      .attr(BypassFreezeDamageReductionAbAttr)
      .conditionalAttr(pokemon => !!pokemon.status || pokemon.hasAbility(AbilityId.COMATOSE), StatMultiplierAbAttr, Stat.SPATK, 1.5),
   new Ability(AbilityId.AQUA_HEART, 224)
      .attr(PostSummonWeatherChangeAbAttr, WeatherType.RAIN)
      .attr(PostBiomeChangeWeatherChangeAbAttr, WeatherType.RAIN)
      .conditionalAttr(getWeatherCondition(WeatherType.RAIN, WeatherType.HEAVY_RAIN), StatMultiplierAbAttr, Stat.SPD, 3 / 2),
   new Ability(AbilityId.BIO_PULSE, 224)
      .attr(PostSummonTerrainChangeAbAttr, TerrainType.PSYCHIC)
      .attr(PostBiomeChangeTerrainChangeAbAttr, TerrainType.PSYCHIC)
      .conditionalAttr(getTerrainCondition(TerrainType.PSYCHIC), StatMultiplierAbAttr, Stat.SPD, 3 / 2),
   new Ability(AbilityId.TUNDRA_SPIRIT, 224)
      .attr(PostSummonWeatherChangeAbAttr, WeatherType.SNOW)
      .attr(PostBiomeChangeWeatherChangeAbAttr, WeatherType.SNOW)
      .conditionalAttr(getWeatherCondition(WeatherType.SNOW, WeatherType.HAIL), StatMultiplierAbAttr, Stat.SPATK, 4 / 3),
   new Ability(AbilityId.NATURAL_SOUL, 224)
      .attr(PostSummonTerrainChangeAbAttr, TerrainType.GRASSY)
      .attr(PostBiomeChangeTerrainChangeAbAttr, TerrainType.GRASSY)
      .conditionalAttr(getTerrainCondition(TerrainType.GRASSY), StatMultiplierAbAttr, Stat.ATK, 4 / 3),
  new Ability(AbilityId.DESERT_MIRACLE, 224)
      .attr(PostSummonWeatherChangeAbAttr, WeatherType.SANDSTORM)
      .attr(PostBiomeChangeWeatherChangeAbAttr, WeatherType.SANDSTORM)
      .conditionalAttr(getWeatherCondition(WeatherType.SANDSTORM), StatMultiplierAbAttr, Stat.DEF, 4 / 3)
      .conditionalAttr(getWeatherCondition(WeatherType.SANDSTORM), StatMultiplierAbAttr, Stat.SPDEF, 4 / 3),
  new Ability(AbilityId.MIRACLE_FOG, 224)
      .attr(PostSummonTerrainChangeAbAttr, TerrainType.MISTY,)
      .attr(PostBiomeChangeTerrainChangeAbAttr, TerrainType.MISTY,)
      .conditionalAttr(getTerrainCondition(TerrainType.MISTY,), StatMultiplierAbAttr, Stat.DEF, 4 / 3)
      .conditionalAttr(getTerrainCondition(TerrainType.MISTY,), StatMultiplierAbAttr, Stat.SPDEF, 4 / 3),
 new Ability(AbilityId.PLUVIAFLUX, 224)
      .conditionalAttr(getWeatherCondition(WeatherType.RAIN, WeatherType.HEAVY_RAIN), PostSummonAddBattlerTagAbAttr, BattlerTagType.PLUVIAFLUX, 0, true)
      .attr(PostWeatherChangeAddBattlerTagAttr, BattlerTagType.PLUVIAFLUX, 0, WeatherType.RAIN, WeatherType.HEAVY_RAIN)
      .attr(BoostEnergyTagAttr, BattlerTagType.HIGHEST_STAT_BOOST, (params) => params.pokemon.getHeldItems().some(item => item instanceof BoostEnergyModifier))
      .uncopiable()
      .attr(NoTransformAbilityAbAttr),
 new Ability(AbilityId.NEURO_CHARGE, 224)
      .conditionalAttr(getTerrainCondition(TerrainType.PSYCHIC), PostSummonAddBattlerTagAbAttr, BattlerTagType.NEURO_CHARGE, 0, true)
      .attr(PostTerrainChangeAddBattlerTagAttr, BattlerTagType.NEURO_CHARGE, 0, TerrainType.PSYCHIC)
      .attr(BoostEnergyTagAttr, BattlerTagType.HIGHEST_STAT_BOOST, (params) => params.pokemon.getHeldItems().some(item => item instanceof BoostEnergyModifier))
      .uncopiable()
      .attr(NoTransformAbilityAbAttr),
  new Ability(AbilityId.CRYOSYNTHESIS, 224)
      .conditionalAttr(getWeatherCondition(WeatherType.SNOW, WeatherType.HAIL), PostSummonAddBattlerTagAbAttr, BattlerTagType.CRYOSYNTHESIS, 0, true)
      .attr(PostWeatherChangeAddBattlerTagAttr, BattlerTagType.CRYOSYNTHESIS, 0, WeatherType.SNOW, WeatherType.HAIL)
      .attr(BoostEnergyTagAttr, BattlerTagType.HIGHEST_STAT_BOOST, (params) => params.pokemon.getHeldItems().some(item => item instanceof BoostEnergyModifier))
      .uncopiable()
      .attr(NoTransformAbilityAbAttr),
 new Ability(AbilityId.PHYTONCIDE, 224)
      .conditionalAttr(getTerrainCondition(TerrainType.GRASS), PostSummonAddBattlerTagAbAttr, BattlerTagType.PHYTONCIDE, 0, true)
      .attr(PostTerrainChangeAddBattlerTagAttr, BattlerTagType.PHYTONCIDE, 0, TerrainType.GRASS)
      .attr(BoostEnergyTagAttr, BattlerTagType.HIGHEST_STAT_BOOST, (params) => params.pokemon.getHeldItems().some(item => item instanceof BoostEnergyModifier))
      .uncopiable()
      .attr(NoTransformAbilityAbAttr),
  new Ability(AbilityId.PSAMMOSYNTHESIS, 224)
      ...
      .uncopiable()
      .attr(NoTransformAbilityAbAttr),
 new Ability(AbilityId.UNSEEN_FORCE, 224)
      .conditionalAttr(getTerrainCondition(TerrainType.MIST), PostSummonAddBattlerTagAbAttr, BattlerTagType.UNSEEN_FORCE, 0, true)
      .attr(PostTerrainChangeAddBattlerTagAttr, BattlerTagType.UNSEEN_FORCE, 0, TerrainType.MIST)
      .attr(BoostEnergyTagAttr, BattlerTagType.HIGHEST_STAT_BOOST, (params) => params.pokemon.getHeldItems().some(item => item instanceof BoostEnergyModifier))
      .uncopiable()
      .attr(NoTransformAbilityAbAttr), 
  );
