...
export abstract class Move implements Localizable {
  ...
  tailMove(): this {
    this.setFlag(MoveFlags2.TAIL_MOVE, true);
    return this;
  }
...
doesFlagEffectApply({
  ...
  // 2️⃣ heldItems 기반 Modifier 처리
  user.getHeldItems?.().forEach(item => {
    if (
      item instanceof MoveAbilityBypassModifier &&
      item.moveIgnoreFunc(user, this)
    ) {
      abilityEffectsIgnored.value = true;
    }
  });

  return abilityEffectsIgnored.value;
...
return moveAccuracy.value;
  ...
  if (!simulated && target && target instanceof Pokemon) {
    const berryMods = globalScene
      .getModifiers(BerryModifier, target.isPlayer())
      .filter(m => m instanceof BerryModifier && m.pokemonId === target.id);
    ...
    if (firstAttack && firstAttack.result === HitResult.SUPER_EFFECTIVE) {
      ...
      for (const mod of berryMods) {
        ...
        if (resistType && resistType === moveType) {
          ...
          // 🔹 열매 소모 처리
          if (!preserve.value) {
            globalScene.removeHeldItemModifier(target, berryType);
          }
          break;
        }
      }
...
const typeSpecificMoveBooster = globalScene.applyModifier(
  ...
  source.isPlayer(),
  source,
  this.type, // ← move.type이 아니라 this.type!
  power
...
if (typeSpecificMoveBooster) {
  ...
  let baseType: PokemonType = moveData?.type ?? this.type;

  const typeHolder = new NumberHolder(baseType);
  applyAbAttrs("MoveTypeChangeAbAttr", { pokemon: user, simulated, move: this, moveType: typeHolder });
  const actualType = typeHolder.value;

  // 디버그용
  console.debug(`[Priority Debug] move=${this.id}, baseType=${PokemonType[baseType]}, actualType=${PokemonType[actualType]}`);
  ...
  const berryType = user.getHeldBerryType?.();
  if (berryType !== undefined) {
    console.debug(`[Priority Debug] berry=${BerryType[berryType]}, mapType=${PokemonType[typeMap[berryType]]}`);

    if (actualType === typeMap[berryType]) {
      console.debug(`[Priority Berry] ${user.name}의 ${BerryType[berryType]} 발동! Priority +1`);
      priority.value += 1;
      user.tryUseBerry(berryType);
    }
  }
...
export class AttackMove extends Move {
  ...
  setZMoveDamageRate(rate: number): this {
    this.zMoveDamageRate = rate;
    return this;
  }
...
export class MoveEffectAttr extends MoveAttr {
  ...
  // 🟥 1. SheerForce 우선 적용
  const sheerForce = globalScene.getModifiers(SheerForceItemModifier)
  .find(mod => mod.pokemonId === user.id);

// ✅ 직접 확률 제거 로직
if (sheerForce) {
  console.debug(`[DEBUG] SheerForce → ${move.name} 부가효과 제거됨`);
  moveChance.value = 0;
}

  sheerForce?.applyAbAttrs("MoveEffectChanceMultiplierAbAttr", {
    pokemon: user,
  ...
  // ✨ 3. 은빛향로 (SereneGraceItemModifier) 적용
  let sereneGraceItem = globalScene.getModifiers(SereneGraceItemModifier)
    .find(mod => mod.pokemonId === user.id);

  if (!sereneGraceItem) {
    sereneGraceItem = globalScene.applyModifier(SereneGraceItemModifier, user.player, user) as SereneGraceItemModifier | null;
  }
  ...
  if (!selfEffect) {
    ...
    const existingCovertCloak = globalScene.getModifiers(IgnoreMoveEffectsItemModifier)
      .find(mod => mod.pokemonId === target.id);

    const hasCovertCloak =
      existingCovertCloak ||
      (target.isPlayer() &&
        globalScene.applyModifier(IgnoreMoveEffectsItemModifier, target.player, target) as IgnoreMoveEffectsItemModifier | null);
...
export class TargetFractionHpDamageAttr extends FixedDamageAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    switch (user.turnData.hitCount - user.turnData.hitsLeft) {
      case 0:
        this.initialHp = target.hp;
      default:
        (args[0] as NumberHolder).value = toDmgValue(this.initialHp * this.fraction);
        return true;
      case lensCount + 1:
        (args[0] as NumberHolder).value = toDmgValue(target.hp * this.fraction);
        return true;
...
export class RecoilAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (!this.unblockable) {
      ...
      // ▶ 아이템으로 recoil 차단: applyItemAttrs 사용하지 않고 직접 검사
  if (!cancelled.value) {
    const itemModifiers = globalScene.getModifiers(RecoilBoosterModifier);
    for (const mod of itemModifiers) {
      if (mod.pokemonId === user.id) {
        mod.applyBlockRecoil(cancelled);  // 👈 확실하게 직접 호출
      }
    }
  }
...
export class ZHealBeforeMoveAttr extends HealAttr {
  private cutHpStatStageBoostAttr: CutHpStatStageBoostAttr;

  constructor(cutHpStatStageBoostAttr: CutHpStatStageBoostAttr) {
    super();
    this.cutHpStatStageBoostAttr = cutHpStatStageBoostAttr;
  }
...
export class ZSacrificialFullRestoreAttr extends MoveEffectAttr {
  protected restorePP: boolean;
  protected moveMessage: string;
...
export class PursuitInterceptAttr extends MoveEffectAttr {
  constructor() {
    super(true);
  }
  ...
  /**
   * 교체 직전에 호출되는 함수
   * @param user Pursuit를 사용하는 포켓몬
   * @param target 교체하려는 포켓몬
   */
  public onSwitchOutAsync(user: Pokemon, target: Pokemon): Promise<void> {
    return new Promise(resolve => {
      ...
      const originalPriority = move.priority;

      // 교체 직전 → 위력 2배, 우선도 +3
      move.power = originalPower * 2;
      move.priority = originalPriority + 3;

      // move 실행
      globalScene.executeMove(user, target, move, () => {
        // move 완료 후 원상복구
        move.power = originalPower;
        move.priority = originalPriority;

        resolve(); // Promise 완료 → preSummon() 계속 진행
      });
    });
  }
}
...
export class BypassFreezeDamageReductionAttr extends MoveAttr {
  /** Prevents the move's damage from being reduced by freeze
   * @param user N/A
   * @param target N/A
   * @param move {@linkcode Move} with this attribute
   * @param args [0] {@linkcode BooleanHolder} for freezeDamageReductionCancelled
   * @returns true if the function succeeds
   */
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    (args[0] as BooleanHolder).value = true;
...
export class ExclusiveWeatherChangeAttr extends MoveEffectAttr {
  private weatherType: WeatherType;
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    return globalScene.arena.trySetExclusiveWeather(this.weatherType, true, user, true); // ignoreAnim = true, force = true
  }

  getCondition(): MoveConditionFunc {
    return (user, target, move) => true; // 항상 발동 가능
  }

  getUserBenefitScore(user: Pokemon, target: Pokemon, move: Move): number {
    // 현재 날씨가 없으면 더 높은 점수
    return globalScene.arena.weather ? 3 : 6;
  }
}
...
export class ExclusiveTerrainChangeAttr extends MoveEffectAttr {
  private terrainType: TerrainType;
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    return globalScene.arena.trySetExclusiveTerrain(this.terrainType, true, user, true); // force = true
  }

  getCondition(): MoveConditionFunc {
    return (user, target, move) => true; // 언제든 발동 가능
  }

  getUserBenefitScore(user: Pokemon, target: Pokemon, move: Move): number {
    // 기본 점수는 동일하게 유지
    return globalScene.arena.terrain ? 3 : 6;
  }
}
...
export class InstantChargeAttr extends MoveAttr {
  ...
  override apply(user: Pokemon, target: Pokemon | null, move: Move, args: any[]): boolean {
    ...
    const conditionResult = this.condition(user, move);

    if (conditionResult) {
      instantCharge.value = true;

      const items = user.getHeldItems();

      const powerHerb = items.find(item => item instanceof InstantChargeItemModifier);
      if (powerHerb) {
...
export class MultiStatStageChangeAttr extends MoveEffectAttr {
  public stats: BattleStat[];
  public stages: number;
  public moveTarget: MoveTarget;
  protected override options?: StatStageChangeAttrOptions;
    ...
    if (chance < 0 || chance >= 100 || user.randBattleSeedInt(100) < chance) {
      globalScene.unshiftPhase(
        new StatStageChangePhase(
          target.getBattlerIndex(),  // 반드시 target 인덱스 사용
          this.selfTarget,
          this.stats,
          this.stages,
          this.showMessage
...
export class ZResetStatsAttr extends MoveEffectAttr {
  private targetAllPokemon: boolean;
...
export class MovePowerMultiplierAttr extends VariablePowerAttr {
  private basePowerMultiplier: number | ((user: Pokemon, target: Pokemon, move: Move) => number);
  private dynamaxPowerMultiplier: number | ((user: Pokemon, target: Pokemon, move: Move) => number);
  ...
  constructor(
    ...
    this.dynamaxPowerMultiplier = dynamaxMultiplier;
  }

  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    const power = args[0] as NumberHolder;

    // ✅ getter 프로퍼티 그대로 접근 (메서드 호출 아님)
    const isDynamaxed = user.isDynamaxed ?? false;
...
export class SplashZBoostAccuracyAttr extends VariableAccuracyAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    const accuracy = args[0] as NumberHolder;
    console.log(`[SplashZBoostAccuracyAttr] Called for move=${move.id}, user=${user.id}`);

    // ❌ move.user는 아직 설정되지 않았을 수 있으므로 검사하지 않음
...
export class PhotonGeyserCategoryAttr extends VariableMoveCategoryAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const atk = user.getEffectiveStat(Stat.ATK, target, move);
    const spatk = user.getEffectiveStat(Stat.SPATK, target, move);
...
export class ShellSideArmCategoryAttr extends VariableMoveCategoryAttr {
  apply(user: Pokemon, target: Pokemon | undefined, move: Move, args: any[]): boolean {
    const category = args[0] as NumberHolder;

    // target이 없는 경우 안전하게 특수 처리
    if (!target) {
      category.value = MoveCategory.SPECIAL;
...
export class NeutralDamageAgainstDarkTypeMultiplierAttr extends VariableMoveTypeMultiplierAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    const multiplier = args[0] as NumberHolder;
    // 악타입 대상일 경우 첫 히트는 항상 1배 대미지
    if (target.isOfType(PokemonType.DARK)) {
      multiplier.value = 1;
    }
    return true;
  }
...
export class DragonTypeMultiplierAttr extends VariableMoveTypeMultiplierAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    const multiplier = args[0] as NumberHolder;
    multiplier.value *= target.getAttackTypeEffectiveness(PokemonType.DRAGON, user);
    return true;
  }
}
...
export class UproarAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (!user.getTag(BattlerTagType.UPROAR) && user.getMoveQueue().length === 0) {
      const turnCount = user.randBattleSeedIntRange(1, 4); // 2~5턴 지속
      for (let i = 0; i < turnCount; i++) {
        user.pushMoveQueue({
          move: move.id,
      ...
      user.lapseTag(BattlerTagType.UPROAR);
    }
...
// RolloutAttr.ts
export class RolloutAttr extends MoveEffectAttr {
  constructor() {
    super(true, { lastHitOnly: true });
  }
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    let rolloutTag = user.getTag(BattlerTagType.ROLLOUT);

    // 새 Rollout 시작 조건
    if (!rolloutTag) {
      // 새 Rollout 시작, stage 1
      rolloutTag = new RolloutTag(1, move.id, user.id);
      user.addTag(BattlerTagType.ROLLOUT, rolloutTag);

      // MoveQueue 초기화 후 4턴 고정 추가 (총 5회)
      user.summonData.moveQueue = [];
      for (let i = 0; i < 4; i++) {
        user.pushMoveQueue({
          move: move.id,
          targets: [target.getBattlerIndex()],
      ...
      console.log(`[RolloutAttr] 새 Rollout 시작: stage=1, moveQueueLength=${user.getMoveQueue().length}`);
    } else {
      // 이미 진행 중이면 lapseTag로 stage 증가
      user.lapseTag(BattlerTagType.ROLLOUT);
    }

    // 현재 stage 기반 위력 계산
    const stage = rolloutTag?.stage || 1;
    move.basePower = 30 * stage;
    console.log(`[RolloutAttr] apply 호출: stage=${stage}, basePower=${move.basePower}, moveQueueLength=${user.getMoveQueue().length}`);

    // 공격 성공 여부
    const hitResult = args[0]?.hitResult;

    if (hitResult && hitResult !== HitResult.MISS && hitResult !== HitResult.IMMUNE) {
      // 성공 시 stage 증가 (최대 5)
      rolloutTag.stage = Math.min(stage + 1, 5);

      // 최대 5회 도달 시 상태 해제
      if (rolloutTag.stage >= 5) {
        console.log(`[RolloutAttr] 최대 5회 완료! Rollout 종료`);
        user.removeTag(BattlerTagType.ROLLOUT);
        user.summonData.moveQueue = [];
      }
    } else if (hitResult === HitResult.MISS || hitResult === HitResult.IMMUNE) {
      // 공격 실패 시 강제 캔슬
      console.log(`[RolloutAttr] 공격 실패! Rollout 태그 제거, MoveQueue 초기화, basePower 초기화`);
      user.removeTag(BattlerTagType.ROLLOUT);
      user.summonData.moveQueue = [];
      move.basePower = 30;
    }
...
export class AddBattlerTagAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
  console.log(`[DEBUG][AddBattlerTagAttr] called for ${BattlerTagType[this.tagType]} by ${user.name} → ${target.name}`);

  if (!super.apply(user, target, move, args)) {
    console.log("[DEBUG][AddBattlerTagAttr] super.apply returned false");
    return false;
  }

  const moveChance = this.getMoveChance(user, target, move, this.selfTarget, true);
  console.log(`[DEBUG] moveChance=${moveChance}, selfTarget=${this.selfTarget}`);
...
export class ZStatStageChangeAttr extends MoveEffectAttr {
  public stats: BattleStat[];
  public stages: number;
...
export class ZMoveEffectAttr extends MoveEffectAttr {
  private effect: MoveEffectAttr;

  constructor(effect: MoveEffectAttr) {
    super(true); // selfTarget은 무시, effect 내부에서 처리
    this.effect = effect;
  }
...
export class LeechSeedAttr extends AddBattlerTagAttr {
  constructor() {
    super(BattlerTagType.SEEDED, false, false, 1, 1);
  }
}
...
export class ZCurseAttr extends MoveEffectAttr {
  private curseAttr: CurseAttr;

  constructor() {
    super();
    this.curseAttr = new CurseAttr();
  }
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (!hasZAccess) {
      console.log(`[ZCurseAttr] Z링/파워링 없음 - CurseAttr 기능 실행`);
      // Z링 없으면 일반 CurseAttr 기능 실행
      return this.curseAttr.apply(user, target, move, args);
    }

    // Z링/파워링 있을 때 ZCurseAttr 로직 실행
    if (user.getTypes(true).includes(PokemonType.GHOST)) {
      if (target.getTag(BattlerTagType.CURSED)) {
    ...
    globalScene.phaseManager.unshiftNew(
      ...
      globalScene.unshiftPhase(new StatStageChangePhase(user.getBattlerIndex(), true, [Stat.ATK, Stat.DEF, Stat.SPATK, Stat.SPDEF], 2));
      globalScene.unshiftPhase(new StatStageChangePhase(user.getBattlerIndex(), true, [Stat.SPD], -2));
      return true;
    }
  }
}
...
export class ZMoveAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    // Protect 무시 플래그를 move에 직접 설정
    if (typeof move.flags === "number") {
      move.flags |= MoveFlags.IGNORE_Z_PROTECT;
    } else {
...
export class MaxGuardProtectAttr extends AddBattlerTagAttr {
  constructor(tagType: BattlerTagType = BattlerTagType.MAX_GUARD_PROTECTED) {
    super(tagType, true);
  }
...
export class AddArenaTagAttr extends MoveEffectAttr {
  ...
  if ((move.chance < 0 || move.chance === 100 || user.randBattleSeedInt(100) < move.chance) &&
    ...
    // ✅ 빛의점토 효과 적용 (리플렉트, 빛의장막, 오로라베일에 한함)
    if ([ArenaTagType.REFLECT, ArenaTagType.LIGHT_SCREEN, ArenaTagType.AURORA_VEIL].includes(this.tagType)) {
      const modifiers = globalScene.getModifiers(WeakenMoveScreenModifier)
        .filter(mod => mod.pokemonId === user.id);

      for (const mod of modifiers) {
        // 적용 조건이 있다면 mod.apply(...)로 걸러내도 좋음
        duration += 2 * mod.stackCount;
      }
    }
...
export class ZMoveOnlyArenaTagAttr extends AddArenaTagAttr {
  override apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    const genericMods = globalScene.getModifiers(GenericZMoveAccessModifier);
    const exclusiveMods = globalScene.getModifiers(ExclusiveZMoveAccessModifier);
    const hasZAccess = genericMods.length > 0 || exclusiveMods.length > 0;
...
export interface FollowAttackAttrOptions {
  multiplier: number;
}

export class FollowAttackAttr extends MoveEffectAttr {
  private multiplier: number;

  constructor(options: FollowAttackAttrOptions) {
    super(true); // true = 대상에게 적용
  ...
  // Move 자체로는 효과 없음; ForceSwitchOutAttr에서 처리
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    return true;
  }
}
...
export class GMaxSmashAttr extends MoveEffectAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    // 특성이 이미 억제됐더라도 거다이융격은 항상 성공
    return true;
  }

  /** 기술은 항상 나가되, 효과는 조건에 따라 발동 */
  getCondition(): MoveConditionFunc {
...
const failIfDampCondition: MoveConditionFunc = (user, target, move) => {
  ...
  if (hasDampIncense || cancelled.value) {
    ...
    // Damp Incense가 적용되었거나, 폭발 차단 상태가 true일 때
    globalScene.phaseManager.queueMessage(
...
export class ResistLastMoveTypeAttr extends MoveEffectAttr {
  ...
  /**
   * User changes its type to a random type that resists the target's last used valid move
  ...
  /**
   * Retrieve the types resisting a given type. Uses type effectiveness, abilities, items, and game mode
   * @returns Array of valid PokemonTypes or empty array if none
   */
  ...
  /**
   * Checks if the move can be used: requires the target has at least one valid last move
   */
  getCondition(): MoveConditionFunc {
    return (user, target, move) => {
      return target.getLastXMoves(1).some(tm => tm.move !== MoveId.NONE && !tm.failed);
    };
  }
}
...
const MoveAttrs = Object.freeze({
  ...
  ExclusiveWeatherChangeAttr,
  ExclusiveTerrainChangeAttr,
  ZStatStageChangeAttr
});
...
export function initMoves() {
  allMoves.push(
    ...
    new AttackMove(MoveId.PAY_DAY, PokemonType.NORMAL, MoveCategory.PHYSICAL, 40, 100, 20, -1, 0, 1)
      .attr(MoneyAttr)
      .makesContact(false),
    new AttackMove(MoveId.FIRE_PUNCH, PokemonType.FIRE, MoveCategory.PHYSICAL, 80, 100, 15, 10, 0, 1)
    ...
    new AttackMove(MoveId.JUMP_KICK, PokemonType.FIGHTING, MoveCategory.PHYSICAL, 100, 95, 10, -1, 0, 1)
      ...
      .recklessMove()
      .kickMove(),
    new AttackMove(MoveId.ROLLING_KICK, PokemonType.FIGHTING, MoveCategory.PHYSICAL, 80, 100, 15, 30, 0, 1)
      .attr(FlinchAttr)
      .spinMove()
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .kickMove(),
    ...
    new AttackMove(MoveId.POISON_STING, PokemonType.POISON, MoveCategory.PHYSICAL, 15, 100, 35, 30, 0, 1)
      ...
      .makesContact(false),
    new AttackMove(MoveId.TWINEEDLE, PokemonType.BUG, MoveCategory.PHYSICAL, 50, 100, 20, 20, 0, 1)
      .attr(MultiHitAttr, MultiHitType._2)
      .spearMove()
      .attr(StatusEffectAttr, StatusEffect.POISON)
      .makesContact(false),
    ...
    new AttackMove(MoveId.DRILL_PECK, PokemonType.FLYING, MoveCategory.PHYSICAL, 80, 100, 20, -1, 0, 1)
      .drillMove()
      .beakMove(),
    new AttackMove(MoveId.SUBMISSION, PokemonType.FIGHTING, MoveCategory.PHYSICAL, 120, 100, 20, -1, 0, 1)
      .attr(RecoilAttr, false, 0.33)
      .wheelMove()
      .recklessMove(),
    ...
    new AttackMove(MoveId.RAZOR_LEAF, PokemonType.GRASS, MoveCategory.PHYSICAL, 75, 100, 25, -1, 0, 1)
      .attr(HighCritAttr)
      .makesContact(false)
      .slicingMove()
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    ...
    new StatusMove(MoveId.THUNDER_WAVE, PokemonType.ELECTRIC, 90, 20, -1, 0, 1)
      ...
      .reflectable(),
    new AttackMove(MoveId.THUNDER, PokemonType.ELECTRIC, MoveCategory.SPECIAL, 120, 70, 10, 30, 0, 1)
      .attr(StatusEffectAttr, StatusEffect.PARALYSIS)
    ...
    new StatusMove(MoveId.POISON_GAS, PokemonType.POISON, 90, 40, -1, 0, 1)
      ...
      .attr(ZStatStageChangeAttr, [Stat.DEF], 1, true)
      .reflectable(),
    new AttackMove(MoveId.BARRAGE, PokemonType.NORMAL, MoveCategory.PHYSICAL, 25, 85, 20, -1, 0, 1)
      .attr(MultiHitAttr)
      .throwMove()
      .makesContact(false)
      .ballBombMove(),
    ...
    new AttackMove(MoveId.EXPLOSION, PokemonType.NORMAL, MoveCategory.PHYSICAL, 250, 100, 5, -1, 0, 1)
      ...
      .makesContact(false)
      .target(MoveTarget.ALL_NEAR_OTHERS),
    new AttackMove(MoveId.FURY_SWIPES, PokemonType.NORMAL, MoveCategory.PHYSICAL, 25, 85, 15, -1, 0, 1)
      .clawMove()
      .attr(MultiHitAttr),
    new AttackMove(MoveId.BONEMERANG, PokemonType.GROUND, MoveCategory.PHYSICAL, 50, 100, 10, -1, 0, 1)
      .attr(MultiHitAttr, MultiHitType._2)
      .boomerangMove()
      .makesContact(false),
    new SelfStatusMove(MoveId.REST, PokemonType.PSYCHIC, -1, 5, -1, 0, 1)
      ...
      .triageMove(),
    new AttackMove(MoveId.ROCK_SLIDE, PokemonType.ROCK, MoveCategory.PHYSICAL, 80, 90, 10, 30, 0, 1)
      .attr(FlinchAttr)
      .throwMove()
      .makesContact(false)
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    ...
    new StatusMove(MoveId.SKETCH, PokemonType.NORMAL, -1, 1, -1, 0, 2)
      ...
      .target(MoveTarget.NEAR_OTHER)
      .attr(SketchAttr),
    new AttackMove(MoveId.TRIPLE_KICK, PokemonType.FIGHTING, MoveCategory.PHYSICAL, 20, 90, 10, -1, 0, 2)
    ...
    new StatusMove(MoveId.SPIKES, PokemonType.GROUND, -1, 20, -1, 0, 2)
      ...
      .reflectable(),
    new AttackMove(MoveId.ZAP_CANNON, PokemonType.ELECTRIC, MoveCategory.SPECIAL, 150, 50, 5, 100, 0, 2)
      .attr(StatusEffectAttr, StatusEffect.PARALYSIS)
      .ballBombMove(),
    new StatusMove(MoveId.FORESIGHT, PokemonType.NORMAL, -1, 40, -1, 0, 2)
      .attr(ExposedMoveAttr, BattlerTagType.IGNORE_GHOST)
      .attr(ZStatStageChangeAttr, BattlerTagType.SPLASH_Z_CRIT_BOOST, [], 0, true)
      .ignoresSubstitute()
      .reflectable(),
    new SelfStatusMove(MoveId.DESTINY_BOND, PokemonType.GHOST, -1, 5, -1, 0, 2)
      .ignoresProtect()
      .attr(DestinyBondAttr)
      .attr(AddBattlerTagAttr, BattlerTagType.Z_CENTER_OF_ATTENTION)
      .condition((user, target, move) => {
    ...
    new StatusMove(MoveId.CHARM, PokemonType.FAIRY, 100, 20, -1, 0, 2)
      ...
      .attr(ZStatStageChangeAttr, [Stat.DEF], 1, true)
      .reflectable(),
    new AttackMove(MoveId.ROLLOUT, PokemonType.ROCK, MoveCategory.PHYSICAL, 30, 100, 20, -1, 0, 2)
      .attr(RolloutAttr, 5)
      .wheelMove()
    ...
    new StatusMove(MoveId.ENCORE, PokemonType.NORMAL, 100, 5, -1, 0, 2)
      ...
      .reflectable(),
    new AttackMove(MoveId.PURSUIT, PokemonType.DARK, MoveCategory.PHYSICAL, 40, 100, 20)
      .partial(),
    ...
    new SelfStatusMove(MoveId.SWALLOW, PokemonType.NORMAL, -1, 10, -1, 0, 3)
      ...
      .edgeCase(),
    new AttackMove(MoveId.HEAT_WAVE, PokemonType.FIRE, MoveCategory.SPECIAL, 80, 100, 10, 30, 0, 3)
      .attr(HealStatusEffectAttr, true, StatusEffect.FREEZE)
      .attr(StatusEffectAttr, StatusEffect.BURN)
      .windMove()
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    ...
    new AttackMove(MoveId.HYPER_VOICE, PokemonType.NORMAL, MoveCategory.SPECIAL, 90, 100, 10, -1, 0, 3)
      .soundBased()
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    new AttackMove(MoveId.POISON_FANG, PokemonType.POISON, MoveCategory.PHYSICAL, 80, 100, 15, 50, 0, 3)
      .attr(StatusEffectAttr, StatusEffect.TOXIC)
      .bitingMove(),
    ...
    new StatusMove(MoveId.FAKE_TEARS, PokemonType.DARK, 100, 20, -1, 0, 3)
      ...
      .attr(ZStatStageChangeAttr, [Stat.SPATK], 1, true)
      .reflectable(),
    new AttackMove(MoveId.AIR_CUTTER, PokemonType.FLYING, MoveCategory.SPECIAL, 80, 100, 25, -1, 0, 3)
      .attr(HighCritAttr)
      .slicingMove()
      .windMove()
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    ...
    new AttackMove(MoveId.WATER_SPOUT, PokemonType.WATER, MoveCategory.SPECIAL, 150, 100, 5, -1, 0, 3)
      .attr(HpPowerAttr)
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    new AttackMove(MoveId.SIGNAL_BEAM, PokemonType.BUG, MoveCategory.SPECIAL, 95, 100, 15, 10, 0, 3)
      .beamMove()
      .lightMove()
      .attr(ConfuseAttr),
    ...
    new AttackMove(MoveId.POISON_JAB, PokemonType.POISON, MoveCategory.PHYSICAL, 80, 100, 20, 30, 0, 4)
      ...
      .beakMove()
      .attr(StatusEffectAttr, StatusEffect.POISON),
    new AttackMove(MoveId.DARK_PULSE, PokemonType.DARK, MoveCategory.SPECIAL, 95, 100, 15, 20, 0, 4)
      .attr(FlinchAttr)
      .pulseMove(),
    ...
    new AttackMove(MoveId.SEED_BOMB, PokemonType.GRASS, MoveCategory.PHYSICAL, 80, 100, 15, -1, 0, 4)
      .makesContact(false)
      .ballBombMove(),
    new AttackMove(MoveId.AIR_SLASH, PokemonType.FLYING, MoveCategory.SPECIAL, 95, 100, 15, 20, 0, 4)
      .attr(FlinchAttr)
      .slicingMove(),
    ...
    new AttackMove(MoveId.DRAGON_RUSH, PokemonType.DRAGON, MoveCategory.PHYSICAL, 100, 85, 10, 20, 0, 4)
      ...
      .attr(HitsTagForDoubleDamageAttr, BattlerTagType.MINIMIZED)
      .attr(FlinchAttr),
    new AttackMove(MoveId.POWER_GEM, PokemonType.ROCK, MoveCategory.SPECIAL, 95, 100, 20, -1, 0, 4),
    ...
    new AttackMove(MoveId.SHADOW_CLAW, PokemonType.GHOST, MoveCategory.PHYSICAL, 70, 100, 15, -1, 0, 4)
      .clawMove()
      .attr(HighCritAttr),
    new AttackMove(MoveId.THUNDER_FANG, PokemonType.ELECTRIC, MoveCategory.PHYSICAL, 80, 100, 15, 10, 0, 4)
      .attr(FlinchAttr)
      .attr(StatusEffectAttr, StatusEffect.PARALYSIS)
      .bitingMove(),
    ...
    new AttackMove(MoveId.CROSS_POISON, PokemonType.POISON, MoveCategory.PHYSICAL, 70, 100, 20, 10, 0, 4)
      ...
      .attr(StatusEffectAttr, StatusEffect.POISON)
      .slicingMove(),
    new AttackMove(MoveId.GUNK_SHOT, PokemonType.POISON, MoveCategory.PHYSICAL, 120, 85, 5, 30, 0, 4)
      .attr(StatusEffectAttr, StatusEffect.POISON)
      .makesContact(false),
    ...
    new AttackMove(MoveId.MAGNET_BOMB, PokemonType.STEEL, MoveCategory.PHYSICAL, 60, -1, 20, -1, 0, 4)
      .makesContact(false)
      .ballBombMove(),
    new AttackMove(MoveId.STONE_EDGE, PokemonType.ROCK, MoveCategory.PHYSICAL, 100, 85, 5, -1, 0, 4)
      .attr(HighCritAttr)
      .makesContact(false),
    ...
    new AttackMove(MoveId.CHATTER, PokemonType.FLYING, MoveCategory.SPECIAL, 65, 100, 20, 100, 0, 4)
      .attr(ConfuseAttr)
      .soundBased(),
    new AttackMove(MoveId.JUDGMENT, PokemonType.NORMAL, MoveCategory.SPECIAL, 120, 100, 10, -1, 0, 4)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
    ...
    new SelfStatusMove(MoveId.HEAL_ORDER, PokemonType.BUG, -1, 5, -1, 0, 4)
      ...
      .attr(ZHealBeforeMoveAttr, 1)
      .triageMove(),
    new AttackMove(MoveId.HEAD_SMASH, PokemonType.ROCK, MoveCategory.PHYSICAL, 150, 85, 5, -1, 0, 4)
    ...
    new AttackMove(MoveId.FLYING_PRESS, PokemonType.FIGHTING, MoveCategory.PHYSICAL, 100, 95, 10, -1, 0, 6)
      ...
      .attr(HitsTagForDoubleDamageAttr, BattlerTagType.MINIMIZED)
      .condition(failOnGravityCondition),
    new StatusMove(MoveId.MAT_BLOCK, PokemonType.FIGHTING, -1, 10, -1, 4, 6)
      .target(MoveTarget.USER_SIDE)
    ...
    new StatusMove(MoveId.FORESTS_CURSE, PokemonType.GRASS, 100, 20, -1, 0, 6)
      ...
      .attr(ZStatStageChangeAttr, [Stat.ATK, Stat.DEF, Stat.SPATK, Stat.SPDEF, Stat.SPD], 1, true)
      .reflectable(),
    new AttackMove(MoveId.PETAL_BLIZZARD, PokemonType.GRASS, MoveCategory.PHYSICAL, 140, 100, 15, -1, 0, 6)
      .windMove()
      .makesContact(false)
      .target(MoveTarget.ALL_NEAR_OTHERS),
    ...
    new AttackMove(MoveId.HYPERSPACE_HOLE, PokemonType.PSYCHIC, MoveCategory.SPECIAL, 80, -1, 5, -1, 0, 6)
      .ignoresProtect()
      .ignoresSubstitute(),
    new AttackMove(MoveId.WATER_SHURIKEN, PokemonType.WATER, MoveCategory.SPECIAL, 25, 100, 20, -1, 1, 6)
      .attr(MultiHitAttr)
    ...
    new AttackMove(MoveId.OBLIVION_WING, PokemonType.FLYING, MoveCategory.SPECIAL, 80, 100, 10, -1, 0, 6)
      ...
      .wingMove()
      .triageMove(),
    new AttackMove(MoveId.THOUSAND_ARROWS, PokemonType.GROUND, MoveCategory.PHYSICAL, 95, 100, 10, -1, 0, 6)
    ...
    new AttackMove(MoveId.THOUSAND_WAVES, PokemonType.GROUND, MoveCategory.PHYSICAL, 110, 100, 10, 100, 0, 6)
      ...
      .makesContact(false)
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    new AttackMove(MoveId.LANDS_WRATH, PokemonType.GROUND, MoveCategory.PHYSICAL, 110, 100, 10, -1, 0, 6)
      .makesContact(false)
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    new AttackMove(MoveId.LIGHT_OF_RUIN, PokemonType.FAIRY, MoveCategory.SPECIAL, 140, 90, 5, -1, 0, 6)
      .attr(RecoilAttr, false, 0.5)
      .recklessMove(),
    new AttackMove(MoveId.ORIGIN_PULSE, PokemonType.WATER, MoveCategory.SPECIAL, 120, 85, 10, -1, 0, 6)
      .pulseMove()
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    ...
    new AttackMove(MoveId.HYPERSPACE_FURY, PokemonType.DARK, MoveCategory.PHYSICAL, 100, -1, 5, -1, 0, 6)
      ...
      .makesContact(false)
      .ignoresProtect(),
    new AttackMove(MoveId.BREAKNECK_BLITZ, PokemonType.NORMAL, MoveCategory.PHYSICAL, 200, -1, 10, -1, 0, 7)
      .ignoresProtect()
      .setZMoveDamageRate(0.25)
      .attr(PhotonGeyserCategoryAttr),
    new AttackMove(MoveId.ALL_OUT_PUMMELING, PokemonType.FIGHTING, MoveCategory.PHYSICAL, 200, -1, 10, -1, 0, 7)
      .ignoresProtect()
      .setZMoveDamageRate(0.25)
      .attr(PhotonGeyserCategoryAttr),
    new AttackMove(MoveId.SUPERSONIC_SKYSTRIKE, PokemonType.FLYING, MoveCategory.PHYSICAL, 200, -1, 10, -1, 0, 7)
      .ignoresProtect()
      .setZMoveDamageRate(0.25)
      .attr(PhotonGeyserCategoryAttr),
    new AttackMove(MoveId.ACID_DOWNPOUR, PokemonType.POISON, MoveCategory.PHYSICAL, 200, -1, 10, -1, 0, 7)
      .ignoresProtect()
      .setZMoveDamageRate(0.25)
      .attr(PhotonGeyserCategoryAttr),
    new AttackMove(MoveId.TECTONIC_RAGE, PokemonType.GROUND, MoveCategory.PHYSICAL, 200, -1, 10, -1, 0, 7)
      .ignoresProtect()
      .setZMoveDamageRate(0.25)
      .attr(PhotonGeyserCategoryAttr),
    new AttackMove(MoveId.CONTINENTAL_CRUSH, PokemonType.ROCK, MoveCategory.PHYSICAL, 200, -1, 10, -1, 0, 7)
      .ignoresProtect()
      .setZMoveDamageRate(0.25)
      .attr(PhotonGeyserCategoryAttr),
    new AttackMove(MoveId.SAVAGE_SPIN_OUT, PokemonType.BUG, MoveCategory.PHYSICAL, 200, -1, 10, -1, 0, 7)
      .ignoresProtect()
      .setZMoveDamageRate(0.25)
      .attr(PhotonGeyserCategoryAttr),
...
new AttackMove(MoveId.CATASTROPIKA, PokemonType.ELECTRIC, MoveCategory.PHYSICAL, 210, -1, 10, -1, 0, 7)
  ...
  .setZMoveDamageRate(0.25),
    ...
    new AttackMove(MoveId.SPIRIT_SHACKLE, PokemonType.GHOST, MoveCategory.PHYSICAL, 80, 100, 10, 100, 0, 7)
      ...
      .arrowMove()
      .makesContact(false),
    new AttackMove(MoveId.DARKEST_LARIAT, PokemonType.DARK, MoveCategory.PHYSICAL, 90, 100, 10, -1, 0, 7)
      .spinMove()
      .attr(IgnoreOpponentStatStagesAttr),
...
new AttackMove(MoveId.CLANGOROUS_SOULBLAZE, PokemonType.DRAGON, MoveCategory.SPECIAL, 185, -1, 10, 100, 0, 7)
  ...
  .target(MoveTarget.ALL_NEAR_ENEMIES),
    ...
    new StatusMove(MoveId.MAGIC_POWDER, PokemonType.PSYCHIC, 100, 20, -1, 0, 8)
      ...
      .powderMove()
      .reflectable(),
    new AttackMove(MoveId.DRAGON_DARTS, PokemonType.DRAGON, MoveCategory.PHYSICAL, 60, 100, 10, -1, 0, 8)
      .attr(MultiHitAttr, MultiHitType._2)
      .arrowMove()
      .makesContact(false)
    ...
    new AttackMove(MoveId.MAX_FLARE, PokemonType.FIRE, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
  .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
  .attr(ExclusiveWeatherChangeAttr, WeatherType.SUNNY),
new AttackMove(MoveId.MAX_FLUTTERBY, PokemonType.BUG, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
...
new AttackMove(MoveId.MAX_LIGHTNING, PokemonType.ELECTRIC, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
...
new AttackMove(MoveId.MAX_STRIKE, PokemonType.NORMAL, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
...
new AttackMove(MoveId.MAX_KNUCKLE, PokemonType.FIGHTING, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
...
new AttackMove(MoveId.MAX_PHANTASM, PokemonType.GHOST, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
...
new AttackMove(MoveId.MAX_HAILSTORM, PokemonType.ICE, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
...
new AttackMove(MoveId.MAX_OOZE, PokemonType.POISON, MoveCategory.PHYSICAL, 150, -1, 100, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
...
new AttackMove(MoveId.MAX_GEYSER, PokemonType.WATER, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
  .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
  .attr(ExclusiveWeatherChangeAttr, WeatherType.SNOW),
new AttackMove(MoveId.MAX_AIRSTREAM, PokemonType.FLYING, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
...
new AttackMove(MoveId.MAX_STARFALL, PokemonType.FAIRY, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
...
new AttackMove(MoveId.MAX_WYRMWIND, PokemonType.DRAGON, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
...
new AttackMove(MoveId.MAX_MINDSTORM, PokemonType.PSYCHIC, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
  .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
  .attr(ExclusiveTerrainChangeAttr, TerrainType.PSYCHIC),
new AttackMove(MoveId.MAX_ROCKFALL, PokemonType.ROCK, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
...
new AttackMove(MoveId.MAX_QUAKE, PokemonType.GROUND, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
...
new AttackMove(MoveId.MAX_DARKNESS, PokemonType.DARK, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
...
new AttackMove(MoveId.MAX_OVERGROWTH, PokemonType.GRASS, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
...
new AttackMove(MoveId.MAX_STEELSPIKE, PokemonType.STEEL, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
  .target(MoveTarget.NEAR_ENEMY)
  .setZMoveDamageRate(0.25)
  .attr(PhotonGeyserCategoryAttr)
  .makesContact(false)
  ...
  .attr(MultiStatStageChangeAttr, [Stat.DEF], 1, MoveTarget.USER_SIDE, true, { moveTarget: MoveTarget.USER_SIDE }),
    ...
    new AttackMove(MoveId.TRIPLE_AXEL, PokemonType.ICE, MoveCategory.PHYSICAL, 20, 90, 10, -1, 0, 8)
      ...
      .attr(MultiHitPowerIncrementAttr, 3)
      .checkAllHits(),
    new AttackMove(MoveId.DUAL_WINGBEAT, PokemonType.FLYING, MoveCategory.PHYSICAL, 50, 100, 10, -1, 0, 8)
      .wingMove()
      .attr(MultiHitAttr, MultiHitType._2),
    ...
    new AttackMove(MoveId.WILDBOLT_STORM, PokemonType.ELECTRIC, MoveCategory.SPECIAL, 100, 85, 10, 20, 0, 8)
      ...
      .windMove()
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    new AttackMove(MoveId.SANDSEAR_STORM, PokemonType.GROUND, MoveCategory.SPECIAL, 100, 85, 10, 20, 0, 8)
      .attr(StormAccuracyAttr)
      .attr(StatusEffectAttr, StatusEffect.BURN)
      .windMove()
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    ...
    new SelfStatusMove(MoveId.TAKE_HEART, PokemonType.PSYCHIC, -1, 10, -1, 0, 8)
     ...
     new AttackMove(MoveId.G_MAX_WILDFIRE, PokemonType.FIRE, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .attr(PhotonGeyserCategoryAttr)
      .makesContact(false)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .setZMoveDamageRate(0.25)
    ...
    new AttackMove(MoveId.G_MAX_BEFUDDLE, PokemonType.BUG, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .attr(PhotonGeyserCategoryAttr)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(MultiStatusEffectAttr, [ StatusEffect.POISON, StatusEffect.PARALYSIS, StatusEffect.SLEEP ]),
    new AttackMove(MoveId.G_MAX_VOLT_CRASH, PokemonType.ELECTRIC, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .attr(PhotonGeyserCategoryAttr)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(StatusEffectAttr, StatusEffect.PARALYSIS),
    new AttackMove(MoveId.G_MAX_GOLD_RUSH, PokemonType.NORMAL, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .attr(PhotonGeyserCategoryAttr)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .setZMoveDamageRate(0.25)
      .attr(ConfuseAttr)
      ...
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(AddArenaTagAttr, ArenaTagType.HAPPY_HOUR, null, true),
    new AttackMove(MoveId.G_MAX_CHI_STRIKE, PokemonType.FIGHTING, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .attr(PhotonGeyserCategoryAttr)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
    ...
    new AttackMove(MoveId.G_MAX_TERROR, PokemonType.GHOST, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .attr(PhotonGeyserCategoryAttr)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(CurseAttr),
    new AttackMove(MoveId.G_MAX_RESONANCE, PokemonType.ICE, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .attr(PhotonGeyserCategoryAttr)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
    ...
    new AttackMove(MoveId.G_MAX_CUDDLE, PokemonType.NORMAL, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .attr(PhotonGeyserCategoryAttr)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(AddBattlerTagAttr, BattlerTagType.INFATUATED),
    new AttackMove(MoveId.G_MAX_REPLENISH, PokemonType.NORMAL, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .attr(PhotonGeyserCategoryAttr)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
    ...
    new AttackMove(MoveId.G_MAX_MALODOR, PokemonType.POISON, MoveCategory.PHYSICAL, 100, -1, 10, 100, 0, 8)
      ...
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(StatusEffectAttr, StatusEffect.TOXIC),
    new AttackMove(MoveId.G_MAX_STONESURGE, PokemonType.WATER, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .attr(PhotonGeyserCategoryAttr)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(AddArenaTrapTagHitAttr, ArenaTagType.STEALTH_ROCK),
    new AttackMove(MoveId.G_MAX_WIND_RAGE, PokemonType.FLYING, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .attr(ClearWeatherAttr, WeatherType.FOG)
      .setZMoveDamageRate(0.25)
      ...
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(RemoveArenaTagsAttr, [ ArenaTagType.MIST, ArenaTagType.SAFEGUARD ], false),
    new AttackMove(MoveId.G_MAX_STUN_SHOCK, PokemonType.ELECTRIC, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
    ...
    new AttackMove(MoveId.G_MAX_FINALE, PokemonType.FAIRY, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
    ...
    new AttackMove(MoveId.G_MAX_DEPLETION, PokemonType.DRAGON, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(ResetStatsAttr, false),
    new AttackMove(MoveId.G_MAX_GRAVITAS, PokemonType.PSYCHIC, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(AddArenaTagAttr, ArenaTagType.GRAVITY, 5),
    new AttackMove(MoveId.G_MAX_VOLCALITH, PokemonType.ROCK, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
    ...
    new AttackMove(MoveId.G_MAX_SANDBLAST, PokemonType.GROUND, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(TrapAttr, BattlerTagType.SAND_TOMB),
    new AttackMove(MoveId.G_MAX_SNOOZE, PokemonType.DARK, MoveCategory.PHYSICAL, 150, -1, 10, 50, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(AddBattlerTagAttr, BattlerTagType.DROWSY, false, true),
    new AttackMove(MoveId.G_MAX_TARTNESS, PokemonType.GRASS, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(StatStageChangeAttr, [ Stat.EVA ], -1),
    new AttackMove(MoveId.G_MAX_SWEETNESS, PokemonType.GRASS, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(PartyStatusCureAttr, i18next.t("moveTriggers:soothingAromaWaftedThroughArea"), AbilityId.SAP_SIPPER),
    new AttackMove(MoveId.G_MAX_SMITE, PokemonType.FAIRY, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(ConfuseAttr),
    new AttackMove(MoveId.G_MAX_STEELSURGE, PokemonType.STEEL, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
    ...
    new AttackMove(MoveId.G_MAX_MELTDOWN, PokemonType.STEEL, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(GMaxSmashAttr),
    new AttackMove(MoveId.G_MAX_FOAM_BURST, PokemonType.WATER, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(StatStageChangeAttr, [ Stat.EVA ], -2),
    new AttackMove(MoveId.G_MAX_CENTIFERNO, PokemonType.Fire, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .attr(TrapAttr, BattlerTagType.FIRE_SPIN),
    new AttackMove(MoveId.G_MAX_VINE_LASH, PokemonType.GRASS, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
    ...
    new AttackMove(MoveId.G_MAX_CANNONADE, PokemonType.WATER, MoveCategory.PHYSICAL, 150, -1, 10, 100, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
    ...
    new AttackMove(MoveId.G_MAX_DRUM_SOLO, PokemonType.GRASS, MoveCategory.PHYSICAL, 150, -1, 10, -1, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .ignoresAbilities(),
    new AttackMove(MoveId.G_MAX_FIREBALL, PokemonType.Fire, MoveCategory.PHYSICAL, 150, -1, 10, -1, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .ignoresAbilities(),
    new AttackMove(MoveId.G_MAX_HYDROSNIPE, PokemonType.WATER, MoveCategory.PHYSICAL, 150, -1, 10, -1, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(PhotonGeyserCategoryAttr)
      .setZMoveDamageRate(0.25)
      .makesContact(false)
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .ignoresAbilities(),
    new AttackMove(MoveId.G_MAX_ONE_BLOW, PokemonType.DARK, MoveCategory.PHYSICAL, 150, -1, 10, -1, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(CritOnlyAttr)
      .attr(PhotonGeyserCategoryAttr)
      .makesContact(false)
      ...
      .attr(MovePowerMultiplierAttr, (user, target, move) => user.isDynamaxed ? 1 : 0.5)
      .ignoresProtect(),
    new AttackMove(MoveId.G_MAX_RAPID_FLOW, PokemonType.WATER, MoveCategory.PHYSICAL, 150, -1, 10, -1, 0, 8)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .attr(CritOnlyAttr)
      .attr(PhotonGeyserCategoryAttr)
      .makesContact(false)
    ...
    new StatusMove(MoveId.REVIVAL_BLESSING, PokemonType.NORMAL, -1, 1, -1, 0, 9)
      ...
      .attr(ZStatStageChangeAttr, [Stat.ATK, Stat.DEF, Stat.SPATK, Stat.SPDEF, Stat.SPD], 3, true)
      .target(MoveTarget.NEAR_OTHER)
      .target(MoveTarget.USER),
    new AttackMove(MoveId.SALT_CURE, PokemonType.ROCK, MoveCategory.PHYSICAL, 40, 100, 15, 100, 0, 9)
      .attr(AddBattlerTagAttr, BattlerTagType.SALT_CURED)
      .makesContact(false),
    new AttackMove(MoveId.TRIPLE_DIVE, PokemonType.WATER, MoveCategory.PHYSICAL, 20, 90, 10, -1, 0, 9)
      .attr(MultiHitAttr, MultiHitType._3)
      .attr(MultiHitPowerIncrementAttr, 3)
      .checkAllHits(),
    ...
    new AttackMove(MoveId.AQUA_CUTTER, PokemonType.WATER, MoveCategory.PHYSICAL, 70, 100, 20, -1, 0, 9)
      ...
      .slicingMove()
      .makesContact(false),
    new AttackMove(MoveId.BLAZING_TORQUE, PokemonType.FIRE, MoveCategory.PHYSICAL, 100, 100, 10, 30, 0, 9)
      .attr(StatusEffectAttr, StatusEffect.BURN)
      .dashMove()
      .wheelMove()
      .makesContact(false),
    new AttackMove(MoveId.WICKED_TORQUE, PokemonType.DARK, MoveCategory.PHYSICAL, 100, 100, 10, 10, 0, 9)
      .attr(StatusEffectAttr, StatusEffect.SLEEP)
      .dashMove()
      .wheelMove()
      .makesContact(false),
    ...
    new AttackMove(MoveId.SPIN_ATTACK, PokemonType.NORMAL, MoveCategory.PHYSICAL, 80, 100, 15, 0, 0, 224)
      ...
      .spinMove()
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    new AttackMove(MoveId.BLADE_STORM, PokemonType.NORMAL, MoveCategory.PHYSICAL, 140, 100, 5, 0, 0, 224)
      .slicingMove()
      .spinMove()
      .target(MoveTarget.ALL_NEAR_OTHERS),
    new AttackMove(MoveId.DOUBLE_PUNCH, PokemonType.NORMAL, MoveCategory.PHYSICAL, 80, 100, 15, 0, 0, 224)
      .punchingMove()
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    new AttackMove(MoveId.MIGHTY_PUNCHER, PokemonType.NORMAL, MoveCategory.PHYSICAL, 140, 85, 5, 0, 0, 224)
      .punchingMove()
      .target(MoveTarget.ALL_NEAR_OTHERS),
    ...
    new AttackMove(MoveId.SPECTRAL_CRASH, PokemonType.GHOST, MoveCategory.PHYSICAL, 120, 100, 15, -1, 0, 224)
      .attr(RecoilAttr, false, 0.33)
      .recklessMove(),
    new AttackMove(MoveId.JET_CRASH, PokemonType.FLYING, MoveCategory.PHYSICAL, 120, 100, 15, -1, 0, 224)
      .attr(RecoilAttr, false, 0.33)
      .recklessMove(),
    new AttackMove(MoveId.WICKED_CRASH, PokemonType.DARK, MoveCategory.PHYSICAL, 120, 100, 15, 10, 0, 224)
      .attr(RecoilAttr, false, 0.33)
      .attr(FlinchAttr)
      .recklessMove(),
    new AttackMove(MoveId.GEO_BRAKE, PokemonType.GROUND, MoveCategory.PHYSICAL, 120, 100, 15, -1, 0, 224)
      .attr(RecoilAttr, false, 0.33)
      .recklessMove(),
    new AttackMove(MoveId.BRAVE_BEETLE, PokemonType.BUG, MoveCategory.PHYSICAL, 120, 100, 15, -1, 0, 224)
      .attr(RecoilAttr, false, 0.33)
      .recklessMove(),
    new AttackMove(MoveId.WYVERN_TACKLE, PokemonType.DRAGON, MoveCategory.PHYSICAL, 120, 100, 15, -1, 0, 224)
      .attr(RecoilAttr, false, 0.33)
      .recklessMove(),
    new AttackMove(MoveId.PSYCHIC_TACKLE, PokemonType.PSYCHIC, MoveCategory.PHYSICAL, 120, 100, 15, -1, 0, 224)
      .attr(RecoilAttr, false, 0.33)
      .recklessMove(),
    ...
    new AttackMove(MoveId.COLD_CRASH, PokemonType.ICE, MoveCategory.PHYSICAL, 120, 100, 15, 10, 0, 224)
      ...
      .attr(StatusEffectAttr, StatusEffect.FROSTBITE)
      .recklessMove(),
    new AttackMove(MoveId.IRON_TACKLE, PokemonType.STEEL, MoveCategory.PHYSICAL, 120, 100, 15, -1, 0, 224)
      .attr(RecoilAttr, false, 0.33)
      .recklessMove(),
    new AttackMove(MoveId.VENOM_CRASH, PokemonType.POISON, MoveCategory.PHYSICAL, 120, 100, 15, 10, 0, 224)
      .attr(RecoilAttr, false, 0.33)
      .attr(StatusEffectAttr, StatusEffect.TOXIC)
      .recklessMove(),
    new AttackMove(MoveId.MAGICAL_EDGE, PokemonType.FAIRY, MoveCategory.PHYSICAL, 120, 100, 15, 10, 0, 224)
      .attr(RecoilAttr, false, 0.33)
      .attr(ConfuseAttr)
      .recklessMove(),
    ...
    new AttackMove(MoveId.LIGHTNING_ARROW, PokemonType.ELECTRIC, MoveCategory.SPECIAL, 80, 100, 5, -1, 2, 224)
      .makesContact(false)
      .arrowMove(),
    new AttackMove(MoveId.BLAZE_ARROW, PokemonType.FIRE, MoveCategory.PHYSICAL, 80, 100, 5, -1, 2, 224)
      .makesContact(false)
      .arrowMove(),
    ...
    new AttackMove(MoveId.MAGICAL_ARROW, PokemonType.FAIRY, MoveCategory.PHYSICAL, 80, 100, 5, -1, 2, 224)
      .makesContact(false)
      .arrowMove(),
    new AttackMove(MoveId.SPARKLING_CANNON, PokemonType.FAIRY, MoveCategory.SPECIAL, 80, 100, 5, -1, 2, 224),
    new AttackMove(MoveId.NIGHT_ARROW, PokemonType.GHOST, MoveCategory.PHYSICAL, 80, 100, 5, -1, 2, 224)
      .makesContact(false)
      .arrowMove(),
    ...
    new AttackMove(MoveId.ROCK_SPEAR, PokemonType.ROCK, MoveCategory.PHYSICAL, 80, 100, 5, -1, 2, 224)
      .spearMove()
      .beakMove(),
    new AttackMove(MoveId.METEOR_ACCEL, PokemonType.ROCK, MoveCategory.SPECIAL, 80, 100, 5, -1, 2, 224),
    ...
    new AttackMove(MoveId.SHADOW_BLADE, PokemonType.GHOST, MoveCategory.PHYSICAL, 90, 100, 15, -1, 0, 224)
      .attr(IgnoreOpponentStatStagesAttr)
      .slicingMove(),
    new AttackMove(MoveId.FEATHER_BLADE, PokemonType.FLYING, MoveCategory.PHYSICAL, 90, 100, 15, -1, 0, 224)
      .attr(IgnoreOpponentStatStagesAttr)
      .slicingMove(),
    new AttackMove(MoveId.DRAGON_BLADE, PokemonType.DRAGON, MoveCategory.PHYSICAL, 90, 100, 15, -1, 0, 224)
      .attr(IgnoreOpponentStatStagesAttr)
      .slicingMove(),
    new AttackMove(MoveId.ANCIENT_SLASH, PokemonType.ROCK, MoveCategory.PHYSICAL, 90, 100, 15, -1, 0, 224)
      .attr(IgnoreOpponentStatStagesAttr)
      .slicingMove(),
    new AttackMove(MoveId.GEO_BLADE, PokemonType.GROUND, MoveCategory.PHYSICAL, 90, 100, 15, -1, 0, 224)
      .attr(IgnoreOpponentStatStagesAttr)
      .slicingMove(),
    new AttackMove(MoveId.METAL_BLADE, PokemonType.STEEL, MoveCategory.PHYSICAL, 90, 100, 15, -1, 0, 224)
      .attr(IgnoreOpponentStatStagesAttr)
      .slicingMove(),
    new AttackMove(MoveId.ICICLE_SWORD, PokemonType.ICE, MoveCategory.PHYSICAL, 90, 100, 15, -1, 0, 224)
      .attr(IgnoreOpponentStatStagesAttr)
      .slicingMove(),
    new AttackMove(MoveId.MAGIC_BLADE, PokemonType.FAIRY, MoveCategory.PHYSICAL, 90, 100, 15, -1, 0, 224)
      .attr(IgnoreOpponentStatStagesAttr)
      .slicingMove(),
    new AttackMove(MoveId.LIGHTNING_SWORD, PokemonType.ELECTRIC, MoveCategory.PHYSICAL, 90, 100, 15, -1, 0, 224)
      .attr(IgnoreOpponentStatStagesAttr)
      .slicingMove(),
    new AttackMove(MoveId.BIO_CRASH, PokemonType.PSYCHIC, MoveCategory.PHYSICAL, 150, 100, 15, 10, 0, 224)
   ...
   new SelfStatusMove(MoveId.ORB_FORCE, PokemonType.DRAGON, -1, 20, -1, 0, 224)
    ...
    new StatusMove(MoveId.WEIRD_POWDER, PokemonType.GRASS, 80, 10, -1, 0, 224)  //Accuracy from Generations 4-6
      ...
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .reflectable(),
    new AttackMove(MoveId.RED_BLAST, PokemonType.FIRE, MoveCategory.SPECIAL, 100, 100, 15, -1, 0, 224)
      .attr(PhotonGeyserCategoryAttr)
      .attr(IgnoreOpponentStatStagesAttr),
    new AttackMove(MoveId.BLAST_WING, PokemonType.FLYING, MoveCategory.SPECIAL, 75, 100, 10, -1, 0, 224)
      .attr(PhotonGeyserCategoryAttr)
      .attr(DragonTypeMultiplierAttr)
      .wingMove()
      .triageMove(),
    ...
    new AttackMove(MoveId.HYDRO_BOMB, PokemonType.WATER, MoveCategory.SPECIAL, 100, 100, 15, -1, 0, 224)
      .attr(PhotonGeyserCategoryAttr)
      .attr(IgnoreOpponentStatStagesAttr),
    new AttackMove(MoveId.HYDRO_DRAIN, PokemonType.WATER, MoveCategory.SPECIAL, 75, 100, 10, -1, 0, 224)
      .attr(HitHealAttr)
      .triageMove(),
    ...
    new AttackMove(MoveId.BREAK_FANG, PokemonType.FIGHTING, MoveCategory.PHYSICAL, 85, 100, 10, -1, 30, 224)
      .attr(FlinchAttr)
      .bitingMove(),
    new AttackMove(MoveId.MAGICAL_FANG, PokemonType.FAIRY, MoveCategory.PHYSICAL, 85, 100, 10, -1, 0, 224)
      .attr(RemoveScreensAttr)
      .bitingMove(),
    ...
    new AttackMove(MoveId.THUNDER_PULSE, PokemonType.ELECTRIC, MoveCategory.SPECIAL, 65, 100, 20, 30, 2, 224)
      ...
      .attr(CritOnlyAttr)
      .pulseMove(),
    new AttackMove(MoveId.BLAZE_PULSE, PokemonType.FIRE, MoveCategory.SPECIAL, 60, 100, 20, 30, 0, 224)
      .attr(StatusEffectAttr, StatusEffect.BURN)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .pulseMove(),
    new AttackMove(MoveId.ZEN_PULSE, PokemonType.PSYCHIC, MoveCategory.SPECIAL, 80, 100, 20, 30, 0, 224)
      .attr(FlinchAttr)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .pulseMove(),
   ...
   new AttackMove(MoveId.SHADOW_KICK, PokemonType.GHOST, MoveCategory.PHYSICAL, 130, 90, 20, -1, 0, 224)
      ...
      .recklessMove()
      .kickMove(),
   new AttackMove(MoveId.WAVE_KICK, PokemonType.GHOST, MoveCategory.PHYSICAL, 130, 90, 20, -1, 0, 224)
      .attr(MissEffectAttr, crashDamageFunc)
      .attr(NoEffectAttr, crashDamageFunc)
      .condition(failOnGravityCondition)
      .recklessMove()
   ...
   new AttackMove(MoveId.POISON_BOOMERANG, PokemonType.POISON, MoveCategory.PHYSICAL, 60, -1, 20, -1, 0, 224)
      .boomerangMove(),
   new AttackMove(MoveId.STONE_BOOMERANG, PokemonType.POISON, MoveCategory.PHYSICAL, 60, -1, 20, -1, 0, 224)
      .boomerangMove(),
   new AttackMove(MoveId.DARK_BOOMERANG, PokemonType.POISON, MoveCategory.PHYSICAL, 75, 100, 20, -1, 0, 224)
      .boomerangMove(),
   ...
   new AttackMove(MoveId.ANCIENT_SPEAR, PokemonType.ROCK, MoveCategory.PHYSICAL, 130, 100, 5, -1, 0, 224)
      .target(MoveTarget.ALL_NEAR_ENEMIES)
      .spearMove(),
   new AttackMove(MoveId.OCEAN_SPEAR, PokemonType.WATER, MoveCategory.PHYSICAL, 120, 85, 5, -1, 0, 224)
      .spearMove(),
   ...
   new AttackMove(MoveId.IRON_LANCE, PokemonType.STEEL, MoveCategory.PHYSICAL, 130, 100, 5, -1, 0, 224)
    ...
    new AttackMove(MoveId.DARKNESS_HAMMER, PokemonType.DARK, MoveCategory.PHYSICAL, 150, 100, 15, -1, 0, 224)
      .hammerMove(),
    new AttackMove(MoveId.SHADOW_HAMMER, PokemonType.DARK, MoveCategory.PHYSICAL, 150, 100, 15, -1, 0, 224)
      .hammerMove(),
    ...
    new AttackMove(MoveId.FLAME_BARRAGE, PokemonType.FIRE, MoveCategory.PHYSICAL, 25, 100, 20, -1, 0, 224)
      ...
      .makesContact(false)
      .ballBombMove(),
    new AttackMove(MoveId.SNOW_BARRAGE, PokemonType.ICE, MoveCategory.PHYSICAL, 25, 100, 20, -1, 0, 224)
      .attr(MultiHitAttr)
      .throwMove()
      .makesContact(false)
      .ballBombMove(),
    new AttackMove(MoveId.BUG_SLIDE, PokemonType.BUG, MoveCategory.PHYSICAL, 80, 90, 10, 30, 0, 224)
      .attr(FlinchAttr)
      .throwMove()
      .makesContact(false)
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    new AttackMove(MoveId.LIGHTNING_THROW, PokemonType.ELECTRIC, MoveCategory.PHYSICAL, 25, 100, 20, -1, 0, 224)
      .attr(MultiHitAttr)
      .throwMove()
      .makesContact(false),
    new AttackMove(MoveId.SPEAR_THROW, PokemonType.STEEL, MoveCategory.PHYSICAL, 80, 90, 10, 30, 0, 224)
      .attr(FlinchAttr)
      .throwMove()
      .makesContact(false),
    new AttackMove(MoveId.CHARMING_THROW, PokemonType.FAIRY, MoveCategory.PHYSICAL, 80, 90, 10, 30, 0, 224)
      .attr(FlinchAttr)
      .throwMove()
      .makesContact(false),
    new AttackMove(MoveId.BITTER_THROW, PokemonType.GHOST, MoveCategory.PHYSICAL, 80, 90, 10, 30, 0, 224)
      .attr(FlinchAttr)
      .throwMove()
      .makesContact(false),
    new AttackMove(MoveId.DEW_BLAST, PokemonType.WATER, MoveCategory.PHYSICAL, 25, 100, 20, -1, 0, 224)
      .attr(MultiHitAttr)
      .throwMove()
      .makesContact(false),
    new AttackMove(MoveId.DRAGON_THROW, PokemonType.DRAGON, MoveCategory.PHYSICAL, 80, 90, 10, 30, 0, 224)
      .attr(FlinchAttr)
      .throwMove()
      .makesContact(false),
    new AttackMove(MoveId.ZEN_THROW, PokemonType.PSYCHIC, MoveCategory.PHYSICAL, 80, 90, 10, 30, 0, 224)
      .attr(FlinchAttr)
      .throwMove()
      .makesContact(false),
    new AttackMove(MoveId.FLASH_ATTCK, PokemonType.NORMAL, MoveCategory.PHYSICAL, 50, 100, 35, -1, 0, 224)
      .lightMove(),
    new AttackMove(MoveId.LIGHTNING_SPLITE, PokemonType.ELECTRIC, MoveCategory.PHYSICAL, 80, 100, 15, -1, 0, 224)
      .lightMove(),
    ...
    new AttackMove(MoveId.GALE_CANNON, PokemonType.FLYING, MoveCategory.SPECIAL, 95, 100, 15, -1, 2, 224)
      .ballBombMove(),
    new AttackMove(MoveId.ENERGY_DRAIN, PokemonType.FIGHTING, MoveCategory.SPECIAL, 75, 100, 10, -1, 0, 2)
      .attr(HitHealAttr)
      .triageMove(),
    new AttackMove(MoveId.COLORFUL_DRAIN, PokemonType.BUG, MoveCategory.SPECIAL, 75, 100, 10, -1, 0, 2)
      .attr(HitHealAttr)
      .triageMove(),
    new AttackMove(MoveId.CHARGE_DRAIN, PokemonType.ELECTRIC, MoveCategory.PHYSICAL, 75, 100, 10, -1, 0, 2)
      .attr(HitHealAttr)
      .triageMove(),
    new AttackMove(MoveId.HEAT_DRAIN, PokemonType.FIRE, MoveCategory.SPECIAL, 75, 100, 10, -1, 0, 2)
      .attr(HitHealAttr)
      .triageMove(),
    new AttackMove(MoveId.EON_DRAIN, PokemonType.STEEL, MoveCategory.SPECIAL, 75, 100, 10, -1, 0, 2)
      .attr(HitHealAttr)
      .triageMove(),
    new AttackMove(MoveId.DRAIN_GEAR, PokemonType.STEEL, MoveCategory.PHYSICAL, 75, 100, 10, -1, 0, 2)
      .attr(HitHealAttr)
      .triageMove(),
    new AttackMove(MoveId.GEO_DRAIN, PokemonType.GROUND, MoveCategory.SPECIAL, 75, 100, 10, -1, 0, 2)
      .attr(HitHealAttr)
      .triageMove(),
    new AttackMove(MoveId.QUAKE_DRAIN, PokemonType.GROUND, MoveCategory.PHYSICAL, 75, 100, 10, -1, 0, 2)
      .attr(HitHealAttr)
      .triageMove(),
    new AttackMove(MoveId.BLACK_DRAIN, PokemonType.DARK, MoveCategory.SPECIALL, 100, 100, 10, -1, 0, 2)
      .attr(HitHealAttr)
      .triageMove(),
    new AttackMove(MoveId.SOUL_DRAIN, PokemonType.GHOST, MoveCategory.SPECIALL, 100, 100, 10, -1, 0, 2)
      .attr(HitHealAttr)
      .triageMove(),
    new AttackMove(MoveId.DRAIN_CLAW, PokemonType.DRAGON, MoveCategory.PHYSICAL, 75, 100, 10, -1, 0, 2)
      .clawMove()
      .attr(HitHealAttr)
      .triageMove(),
    new AttackMove(MoveId.DRAGON_DRAIN, PokemonType.DRAGON, MoveCategory.SPECIAL, 100, 100, 10, -1, 0, 2)
      .attr(HitHealAttr)
      .triageMove(),
    new AttackMove(MoveId.COLD_DRAIN, PokemonType.ICE, MoveCategory.SPECIAL, 100, 100, 10, -1, 0, 2)
      .attr(HitHealAttr)
      .triageMove(),
    new AttackMove(MoveId.MINERAL_DRAIN, PokemonType.ROCK, MoveCategory.SPECIAL, 100, 100, 10, -1, 0, 2)
      .attr(HitHealAttr)
      .triageMove(),
    new AttackMove(MoveId.MANA_DRAIN, PokemonType.NORMAL, MoveCategory.SPECIAL, 60, 100, 10, -1, 0, 2)
      .attr(HitHealAttr)
      .triageMove(),
    new AttackMove(MoveId.POWER_DRAIN, PokemonType.NORMAL, MoveCategory.PHYSICAL, 60, 100, 10, -1, 0, 2)
      .attr(HitHealAttr)
      .triageMove()
  );
}
