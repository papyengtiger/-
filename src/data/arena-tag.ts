...
import { RoomServiceModifier, AbilityGuardItemModifier } from "../modifier/modifier";
...
export class GMaxSteelsurgeTag extends DamagingTrapTag {
  constructor(sourceId: number, side: ArenaTagSide) {
    super(ArenaTagType.G_MAX_STEELSURGE, Moves.G_MAX_STEELSURGE, sourceId, side, 1);
  }
  ...
  onAdd(arena: Arena, quiet = false): void {
    ...
    if (!quiet && source) {
      globalScene.queueMessage(
        i18next.t("arenaTag:gMaxSteelsurgeOnAdd", {
          opponentDesc: source.getOpponentDescriptor(),
        }),
      );
    }
  ...
  override activateTrap(pokemon: Pokemon, simulated: boolean): boolean {
    ...
    if (damageHpRatio) {
      ...
      const damage = toDmgValue(pokemon.getMaxHp() * damageHpRatio);
      globalScene.queueMessage(
        i18next.t("arenaTag:gMaxSteelsurgeActivateTrap", {
          pokemonNameWithAffix: getPokemonNameWithAffix(pokemon),
        }),
      );
      pokemon.damageAndUpdate(damage, { result: HitResult.INDIRECT });
...
export class TrickRoomTag extends ArenaTag {
  constructor(turnCount: number, sourceId: number, pokemon: Pokemon) {
  ...
  onAdd(_arena: Arena): void {
    ...
    // 모든 포켓몬을 순회하며, RoomServiceModifier 확인
    const allPokemon = playerField.concat(enemyField).filter(p => p.isActive());
    for (const pokemon of allPokemon) {
      if (RoomServiceModifier) {
        const roomService = globalScene.applyModifier(
          RoomServiceModifier,
          pokemon.isPlayer() ? playerField : enemyField,
          pokemon,
        ) as RoomServiceModifier;

        if (roomService) {
          // 룸서비스가 적용되었으므로 'DISABLED' 태그를 제거
          pokemon.removeTag(BattlerTagType.DISABLED);
          globalScene.updateModifiers(pokemon.isPlayer() ? playerField : enemyField);

          return; // 룸서비스가 적용되었으면 더 이상 진행하지 않음
        }
      }
    }
  }
...
export class SuppressAbilitiesTag extends SerializableArenaTag {
  ...
  // beingRemoved은 외부에 노출되지만 직렬화되지 않도록 private field로 관리
  ...
  public override loadTag(
    source: BaseArenaTag & Pick<SuppressAbilitiesTag, "tagType" | "sourceCount">,
  ): void {
    super.loadTag(source);
  ...
  public override onAdd(_arena: Arena): void {
    ...
    for (const p of globalScene.getField(true)) {
      ...
      // 1. 화학변화가스를 낸 포켓몬은 자기 자신을 억제하지 않음
      if (p.id === pokemon.id) {
        p.battleData.canUseAbilities = true;
        continue;
      }

      // 2. 특성가드 아이템 보유자는 억제하지 않음
      const hasAbilityGuard = globalScene
        .getModifiers(AbilityGuardItemModifier)
        .some(mod => mod.pokemonId === p.id);
      if (hasAbilityGuard) {
        p.battleData.canUseAbilities = true;
        continue;
      }
      ...
      // 3. 나머지 포켓몬은 특성 억제
      if (p.battleData?.canUseAbilities !== false) {
        p.battleData.canUseAbilities = false;
        [true, false].forEach(passive =>
          applyOnLoseAbAttrs({ pokemon: p, passive }),
        );
      }
    }
  }
  ...
  public onSourceLeave(arena: Arena): void {
    ...
    if (this.sourceCount <= 0) {
      ...
      // 1개의 소스가 남으면 그 포켓몬의 다른 특성이 다시 활성화되어야 함
      const setter = globalScene
        .getField()
        .filter(p =>
          p?.hasAbilityWithAttr(
      ...
      if (setter) {
        applyOnGainAbAttrs({
          pokemon: setter,
          passive: setter.getAbility().hasAttr(
            "PreLeaveFieldRemoveSuppressAbilitiesSourceAbAttr",
          ),
        });
      }
  ...
  public override onRemove(_arena: Arena, quiet = false): void {
    this.#beingRemoved = true;
    if (!quiet) {
      globalScene.phaseManager.queueMessage(
        i18next.t("arenaTag:neutralizingGasOnRemove"),
      );
    }
    ...
    for (const p of globalScene.getField(true)) {
      ...
      // 특성가드로 보호된 포켓몬은 원래 그대로 두고
      const hasAbilityGuard = globalScene
        .getModifiers(AbilityGuardItemModifier)
        .some(mod => mod.pokemonId === p.id);
      if (hasAbilityGuard) continue;

      // 나머지는 특성 복원
      p.battleData.canUseAbilities = true;
      [true, false].forEach(passive =>
        applyOnGainAbAttrs({ pokemon: p, passive }),
      );
    }
  }
...
export function getArenaTag(
  ...
  switch (tagType) {
    ...
    case ArenaTagType.G_MAX_STEELSURGE:
      return new GMaxSteelsurgeTag(sourceId, side);
...
export type ArenaTagTypeMap = {
  ...
  [ArenaTagType.G_MAX_STEELSURGE]: GMaxSteelsurgeTag;
