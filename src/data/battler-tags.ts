...
import { MentalHerbModifier, GenericZMoveAccessModifier, ExclusiveZMoveAccessModifier } from "../modifier/modifier";
...
export class RolloutTag extends SerializableBattlerTag {
  ...
  constructor(stage: number, sourceMove: MoveId, sourceId: number) {
    super(BattlerTagType.ROLLOUT, BattlerTagLapseType.CUSTOM, 5, sourceMove, sourceId);
    this.stage = Number(stage); // 안전하게 숫자 변환
  }

  onRemove(pokemon: Pokemon): void {
    super.onRemove(pokemon);
    // Rollout 종료 시 moveQueue 초기화
    pokemon.summonData.moveQueue = [];
    console.log(`[RolloutTag] onRemove: moveQueue 초기화, stage 리셋`);
  }
}
...
export class TruantTag extends AbilityBattlerTag {
  ...
  lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    // 🔹 체력 50% 회복
    if (!pokemon.isFullHp()) {
      const healAmount = Math.floor(pokemon.getMaxHp() * 0.5);
      globalScene.phaseManager.unshiftNew(
        "PokemonHealPhase",
        pokemon.getBattlerIndex(),
        healAmount,
        i18next.t("abilityTriggers:truantHeal", {
          pokemonNameWithAffix: getPokemonNameWithAffix(pokemon),
        }),
        true
      );
    ...
    if (pokemon.status && pokemon.status.effect !== StatusEffect.FAINT) {
      globalScene.phaseManager.queueMessage(
        i18next.t("abilityTriggers:truantCureStatus", {
          pokemonNameWithAffix: getPokemonNameWithAffix(pokemon),
          statusName: pokemon.status.effect
        })
      );
...
export function getBattlerTag(
  ...
  switch (tagType) {
    ...
    case BattlerTagType.HIGHEST_STAT_BOOST:
      return new HighestStatBoostTag();
