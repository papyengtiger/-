...
export function isMaxPokemon(pokemon: Pokemon): boolean {
  const maxForms = [
  ...
  const fusionKey = pokemon.getFusionFormKey?.();

  // 폼 체크 + isDynamaxed 체크 추가
  if (!formKey) return false;

  return (maxForms.includes(formKey) || (fusionKey && maxForms.includes(fusionKey))) && pokemon.isDynamaxed;
}
