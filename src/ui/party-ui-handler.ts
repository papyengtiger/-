...
export class PartyUiHandler extends MessageUiHandler {
  ...
  private processToggleLearningOption(pokemon: Pokemon): boolean {
  const ui = this.getUi();
  this.clearOptions();
  ui.playSelect();

  // 토글
  pokemon.pauseLearningMoves = !pokemon.pauseLearningMoves;

  // 옵션 라벨 즉시 갱신
  if (typeof this.updateOptionsWindow === "function") {
    this.updateOptionsWindow();
  }

  this.showText(
    i18next.t(
      pokemon.pauseLearningMoves
        ? "partyUiHandler:pausedLearning"
        : "partyUiHandler:unpausedLearning",
      { pokemonName: getPokemonNameWithAffix(pokemon, false) },
    ),
    undefined,
    () => this.showText("", 0),
  ...
  private processActionButtonForOptions(option: PartyOption) {
    ...
    if (option === PartyOption.TOGGLE_LEARNING) {
      return this.processToggleLearningOption(pokemon);
    }
  ...
  private addCommonOptions(pokemon: Pokemon): void {
  ...
  // 기술 배우기 잠금/해제 옵션: 항상 표시
  this.options.push(PartyOption.TOGGLE_LEARNING);
  }
  ...
  if (formChangeItemModifiers && option >= PartyOption.FORM_CHANGE_ITEM) {
    ...
    optionName = `${pokemon.pauseEvolutions
      ? i18next.t("partyUiHandler:unpauseEvolution")
      : i18next.t("partyUiHandler:pauseEvolution")}`;
  } else if (option === PartyOption.TOGGLE_LEARNING) {
    // pauseLearningMoves 상태에 따라 라벨 바꿈
    optionName = pokemon.pauseLearningMoves
      ? i18next.t("partyUiHandler:unpauseLearning") // 잠금 상태 → "잠금 해제" 라벨
      : i18next.t("partyUiHandler:pauseLearning");  // 잠금 해제 상태 → "잠금" 라벨
  } else {
