...
export interface SystemSaveData {
  ...
  starterAttributes?: StarterAttributes; // 추가
  gameStats: GameStats;
  unlocks: Unlocks;
...
export interface SessionSaveData {
   ...
   */
  playerFaints: number;
  starterPassiveAbilities?: StarterPassiveAbilities;
}
...
export class GameData {
  ...
  public eggProgress: Record<number, { initialized: boolean; hatchWaves: number }>;
  ...
  return {
    ...
    starterAttributes: this.starterAttributes, // 추가: 성격, 특성, 폼 등
  ...
  public initSystem(systemDataStr: string, cachedSystemDataStr?: string): Promise<boolean> {
    ...
    try {
      ...
      if (!systemData.starterData) {
        ...
        // 스타터 속성 병합
if (systemData.starterAttributes) {
  for (const s of Object.keys(systemData.starterAttributes)) {
    const attr: StarterAttributes = systemData.starterAttributes[s];
    if (!this.starterData[s]) continue;
    this.starterData[s] = {
      ...this.starterData[s],
      attributes: {
        ...this.starterData[s].attributes,
        ...attr,
      },
    };
  }
      ...
      this.eggProgress = systemData.eggProgress ?? {};
  ...
  parseSessionData(dataStr: string): SessionSaveData {
    ...
    const sessionData = JSON.parse(dataStr, (k: string, v: any) => {
      ...
      switch (k) {
        ...
        case "starterPassiveAbilities":
  return v as StarterPassiveAbilities;
    ...
    if (sessionData.starterPassiveAbilities) {
  if (!globalScene.runData) globalScene.runData = {};
  globalScene.runData.starterPassiveAbilities = sessionData.starterPassiveAbilities;
}
    return sessionData;
  }
  ...
  for (const speciesId of starterSpeciesIds) {
    starterData[speciesId] = {
      ...
      attributes: {
        ...
        favorite: false,
        nickname: "",
        tera: undefined, // PokemonType이면 undefined로 초기화
      },
    };
  }
  ...
  return new Promise<boolean>(resolve => {
    ...
    if (formIndex > 0) {
      ...
      if (pokemon.species.speciesId === SpeciesId.URSHIFU) {
        if (formIndex === 2) dexEntry.caughtAttr |= globalScene.gameData.getFormAttr(0);
        else if (formIndex === 3) dexEntry.caughtAttr |= globalScene.gameData.getFormAttr(1);
      } else if (pokemon.species.speciesId === SpeciesId.ZYGARDE) {
        if (formIndex === 4) dexEntry.caughtAttr |= globalScene.gameData.getFormAttr(2);
        else if (formIndex === 5) dexEntry.caughtAttr |= globalScene.gameData.getFormAttr(3);
      } else {
        ...
        if (toCurrentFormChanges.length > 0) dexEntry.caughtAttr |= globalScene.gameData.getFormAttr(0);
      }
    }

    // 스타터 속성 업데이트
    if (speciesStarterCosts.hasOwnProperty(species.speciesId)) {
      const starter = this.starterData[species.speciesId];
      ...
      starter.attributes = {
        ...
        // nickname, favorite, tera 등은 필요에 따라 별도로 처리 가능
      };
    }
