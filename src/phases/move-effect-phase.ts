...
export class MoveEffectPhase extends PokemonPhase {
  ...
  if (anySuccess) {
    ...
    // ✅ AbilityGuard 검사 (arena.ignoreAbilities가 켜져있을 때만)
    if (globalScene.arena.ignoreAbilities) {
      for (const target of targets) {
        if (target.getHeldItems().some(item => item instanceof AbilityGuardItemModifier)) {
          console.debug(`[DEBUG] ${target.name}의 AbilityGuard 발동 → ignoreAbilities 해제`);
          globalScene.arena.setIgnoreAbilities(false, user.getBattlerIndex());
          break; // 한 마리만 가드 있어도 전체 무시 해제
        }
      }
    }
  } else {
...
this.postAnimCallback(user, targets, move);
  ...
  if (!target.isFainted() || target.canApplyAbility()) {
    ...
    if (move) {
      // 접촉 기술 체크
      if (move.hasFlag(MoveFlags.MAKES_CONTACT, user, target)) {
        ...
        if (hasRockyHelmet) {
          const maxHp = user.getMaxHp();
          const damage = Math.floor(maxHp / 6);
          user.damageAndUpdate(damage, { result: HitResult.INDIRECT });
  ...
  const modifiers = globalScene.getModifiers(TypeImmunityModifier).filter(mod => mod.pokemonId === target.id);
  ...
  // 🔹 치명타 특성 효과 처리
  const params = { pokemon: target, opponent: user, move: this.move, hitResult, damage: 0 };
  if (wasCritical) {
