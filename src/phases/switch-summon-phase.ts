...
export class SwitchSummonPhase extends SummonPhase {
  ...
  const enemies = this.player ? globalScene.getEnemyField() : globalScene.getPlayerField();
  if (!enemies) return;

  // --- 1️⃣ 교체 직전 Pursuit 대미지 ---
  for (const enemy of enemies) {
    if (!enemy?.moves) continue;
    for (const move of enemy.moves) {
      if (move.hasAttr("PursuitInterceptAttr")) {
        await (move.getAttr("PursuitInterceptAttr") as PursuitInterceptAttr)
          .onSwitchOutAsync(enemy, pokemon);
      }
    }
  }

  // --- 2️⃣ 기존 태그 제거, Substitute 처리, 메시지, 애니메이션 ---
  (this.player ? globalScene.getEnemyField() : globalScene.getPlayerField())
    .forEach(enemyPokemon => enemyPokemon.removeTagsBySourceId(pokemon.id));
