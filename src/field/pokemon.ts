...
export abstract class Pokemon extends Phaser.GameObjects.Container {
  ...
  public rolloutUsedCount?: number;
  ...
  public usedTRs: MoveId[];
  public usedZMoves: MoveId[];
  ...
  constructor(
    ...
    if (dataSource) {
      ...
      this.pauseLearningMoves = dataSource.pauseLearningMoves;
      ...
      this.usedTRs = dataSource.usedTRs ?? [];
      this.usedZMoves = dataSource.usedZMoves ?? [];
...
set isDynamaxed(value: boolean) {
  console.trace(`[isDynamaxed] ${this.name} changed from ${this._isDynamaxed} to ${value}`);
...
// ✅ 상태이상일 때만 발동하는 아이템 (신비한비늘)
  if (this.status !== StatusEffect.NONE) {
    ...
    // ✅ 여기에서 배율 아이템을 적용 (DualStatMultiplierModifier)
if (!ignoreHeldItems) {
  globalScene.applyModifiers(DualStatMultiplierModifier, this.isPlayer(), this, stat, statVal);
  console.debug(`[STAT] ${this.name} ${Stat[stat]} 최종값 = ${statVal.value}`);
}
  ...
  calculateStats(): void {
    ...
    for (const s of PERMANENT_STATS) {
      ...
      if (s === Stat.HP) {
  const wasDynamaxed = this.isDynamaxed;

  const oldHp = this.hp;
  const oldMaxHp = this.stats[Stat.HP]; // ✅ 갱신 전에 반드시 저장
  ...
  } else if (!wasDynamaxed && oldMaxHp > 0) {
    const ratio = oldHp / oldMaxHp;
    this.hp = Math.floor(ratio * statHolder.value);
  }
...
setDynamax(state: boolean) {
 ...
 if ((this.isOnField() || suppressOffField) && suppressAbilitiesTag && !suppressAbilitiesTag.beingRemoved) {
    const hasAbilityGuard = globalScene.getModifiers(AbilityGuardItemModifier).some(mod => mod.pokemonId === this.id);

    // AbilityGuard가 있으면 화학변화가스 우선 처리
    if (hasAbilityGuard) {
      return true;
    }
...
if (source !== this) {
  ...
  .map(defType => {
    ...
    if (move) {
      applyMoveAttrs(
        ...
        this,
        move,
        typeMultiplier, // ✅ multiplier 대신 typeMultiplier 전달
        defType         // ✅ defenderType 대신 defType 사용
      );
    }
    ...
    if (source?.isActive(true) && source.hasAbilityWithAttr("IgnoreTypeImmunityAbAttr")) {
      applyAbAttrs("IgnoreTypeImmunityAbAttr", {
        ...
        simulated,
        moveType,
        defenderType: defType, // ✅ 여기도 defType 전달
      });
    }

    // 2. 아이템 Modifier 직접 적용
    const heldItems = source?.getHeldItems?.();
    ...
    // 3. 면역 무시된 경우 multiplier 값을 1로 설정
  ...
  getAttackDamage({
    ...
    const resistBerryMultiplier = new NumberHolder(1);
...
if (!simulated && this !== source) {
  ...
  const moveTypeForBerry = source.getMoveType(move);
...
let total =
  ...
  resistBerryMultiplier.value; // ✅ 여기서 곱하기!

console.log(`[DEBUG][ResistBerryMult] final = x${resistBerryMultiplier.value}`);

console.log("[DMG] before PG:", total);

// ✅ 현재 기술을 사용하는 포켓몬(source)이 실제로 들고 있는 Modifier만 가져오기
const heldMods = globalScene.findModifiers(
  m => m instanceof PokemonHeldItemModifier && m.pokemonId === source.id
);

// ✅ 각종 타입별 보정 처리 (실제 장착된 Modifier만 적용)
if (heldMods.some(m => m instanceof PunchingGloveModifier) && move.hasFlag(MoveFlags.PUNCHING_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof SlicingMoveModifier) && move.hasFlag(MoveFlags.SLICING_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof BitingMoveModifier) && move.hasFlag(MoveFlags.BITING_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof HeadMoveModifier) && move.hasFlag(MoveFlags.HEAD_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof HornMoveModifier) && move.hasFlag(MoveFlags.HORN_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof KickMoveModifier) && move.hasFlag(MoveFlags.KICK_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof SpearMoveModifier) && move.hasFlag(MoveFlags.SPEAR_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof WingMoveModifier) && move.hasFlag(MoveFlags.WING_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof HammerMoveModifier) && move.hasFlag(MoveFlags.HAMMER_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof ClawMoveModifier) && move.hasFlag(MoveFlags.CLAW_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof PinchMoveModifier) && move.hasFlag(MoveFlags2.PINCH_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof BeakMoveModifier) && move.hasFlag(MoveFlags2.BEAK_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof DashMoveModifier) && move.hasFlag(MoveFlags2.DASH_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof SpinMoveModifier) && move.hasFlag(MoveFlags2.SPIN_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof DrillMoveModifier) && move.hasFlag(MoveFlags2.DRILL_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof WhipMoveModifier) && move.hasFlag(MoveFlags2.WHIP_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof WheelMoveModifier) && move.hasFlag(MoveFlags2.WHEEL_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof TailMoveModifier) && move.hasFlag(MoveFlags2.TAIL_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof ArrowMoveModifier) && move.hasFlag(MoveFlags.ARROW_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof BallBombMoveModifier) && move.hasFlag(MoveFlags.BALLBOMB_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof BoomerangMoveModifier) && move.hasFlag(MoveFlags2.BOOMERANG_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof ThrowMoveModifier) && move.hasFlag(MoveFlags2.THROW_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof PulseMoveModifier) && move.hasFlag(MoveFlags.PULSE_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof BeamMoveModifier) && move.hasFlag(MoveFlags.BEAM_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof LightMoveModifier) && move.hasFlag(MoveFlags2.LIGHT_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof SoundMoveModifier) && move.hasFlag(MoveFlags.SOUND_BASED)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof WindMoveModifier) && move.hasFlag(MoveFlags.WIND_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof DanceMoveModifier) && move.hasFlag(MoveFlags.DANCE_MOVE)) {
  total = Math.floor(total * 1.3);
  move.setFlag(MoveFlags.MAKES_CONTACT, false);
}

if (heldMods.some(m => m instanceof DrainMoveModifier) && move.hasAttr("HitHealAttr")) {
  total = Math.floor(total * 1.5);
}

console.log("[DMG] after PG:", total);

// ✅ 최종 대미지 확정
damage.value = toDmgValue(total);

    // StackingPowerBoosterModifier 적용 (기술 위력 증가)
if (StackingPowerBoosterModifier) {
  globalScene.applyModifiers(StackingPowerBoosterModifier, false, damage);
}

// IgnoreContactItemModifier 적용 (비접촉 효과)
if (IgnoreContactItemModifier) {
  globalScene.applyModifiers(IgnoreContactItemModifier, false, damage);
}

// 대미지 계산하는 부분에 추가하세요!
// 예시: 대미지 계산 후 sheerForce 있으면 곱하기 1.33
const sheerForce = globalScene.getModifiers(SheerForceItemModifier)

if (sheerForce && damage) {
  damage.value = Math.floor(damage.value * 1.3);
}

// ✅ 테크닉벨트 적용 (위력 60 이하 기술의 위력 증가)
if (damage.value <= 60) {
  globalScene.applyModifiers(MovePowerBoostItemModifier, false, damage);
}

// ✅ RecoilBoosterModifier 적용 (반동 기술의 위력 증가)
if (RecoilBoosterModifier && move.hasAttr(("RecoilAttr"))) {
  globalScene.applyModifiers(RecoilBoosterModifier, false, damage, move);
    ...
    // ✅ SturdystoneItemModifier 적용 (체력이 가득 찼을 때 받는 데미지 감소)
if (Math.floor(this.hp) >= Math.floor(this.getMaxHp())) {
  globalScene.applyModifiers(SturdystoneItemModifier, false, damage, move, this, source);
    ...
    // 🟢 여기에 사운드 기술 피해 절반 감소 로직 추가
  const heldModsDefender = globalScene.findModifiers(
    m => m instanceof PokemonHeldItemModifier && m.pokemonId === this.id
  );

  // SoundMoveModifier가 있고, 기술이 SOUND_BASED 플래그를 가진다면 받는 피해 0.5배
  if (heldModsDefender.some(m => m instanceof SoundMoveModifier) && move.hasFlag(MoveFlags.SOUND_BASED)) {
    damage.value = Math.floor(damage.value * 0.5);
    console.log(`[SoundMoveModifier] ${move.name} → 사운드 기술 피해 0.5배 감소`);
  }

if (heldModsDefender.some(m => m instanceof WindMoveModifier) && move.hasFlag(MoveFlags.WIND_MOVE)) {
    damage.value = Math.floor(damage.value * 0.5);
    console.log(`[SoundMoveModifier] ${move.name} → 사운드 기술 피해 0.5배 감소`);
  }
    ...
    const attacker = source; // 공격자
const defender = this;   // 방어자

const mods = globalScene.getModifiers(StackingRiskyPowerBoosterModifier);
const matchedMod = mods.find(mod => mod.pokemonId === attacker.id);
...
/** SuperEffectiveBoosterModifier 적용 (효과가 굉장한 공격일 때) */
if (typeMultiplier > 1) {
...
if (typeMultiplier < 1) {
  ...
  if (this.isDynamaxed) {
    ...
    console.log(`[Dynamax] damage halved: ${damage}`);
  }
  ...
  if (!preventEndure && this.hp - damage <= 0) {
    ...
    if (surviveDamage.value) {
      console.log("[DEBUG][Survive] 발동됨 → HP 1로 조정");
      damage = this.hp - 1;
      this.hp = 1;
    }
  }
  ...
  // ✅ 반격용 피해 기록
  this.lastDamage = Math.min(originalDamage, this.getMaxHp());

  // ✅ 항상 HP 기준 피해 보정 (생존 여부와 무관)
  damage = Math.min(damage, this.hp);

  // ✅ HP 감소
const appliedDamage = Math.min(damage, this.hp);
...
export class PlayerPokemon extends Pokemon {
  ...
  public compatibleZMoves: Moves[];
  public compatibleTrs: Moves[];
  ...
  constructor(
    ...
    this.generateCompatibleZMoves();
    this.generateCompatibleTrs();
  }
  ...
  generateCompatibleZMoves(): void {
    ...
    // 범용 Z기술
    for (const type in typeToZMoveMap) {
      const moveId = typeToZMoveMap[type as unknown as PokemonType];
      if (moveId !== undefined) {
        this.compatibleZMoves.push(moveId);
      }
    }
  ...
  private handleSpecialEvolutions(evolution: SpeciesFormEvolution) {
    ...
    if (evoSpecies?.speciesId === SpeciesId.NINCADA && evolution.speciesId === SpeciesId.NINJASK) {
      ...
      if (validateShedinjaEvo()) {
        ...
        newPokemon.usedTRs = this.usedTRs;
        newPokemon.usedZMoves = this.usedZMoves;
